  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <h2 className="text-lg font-bold text-neutral-800 dark:text-neutral-200">{year}년 연간 정산</h2>
      ...
      <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-xl bg-blue-50 dark:bg-blue-950/40 p-3">
            <p className="text-xs font-semibold text-blue-800 dark:text-blue-300">연 수입</p>
            <p className="mt-1 text-base font-bold text-blue-700 dark:text-blue-300">{formatWon(totalIncome)}</p>
          </div>
          <div className="rounded-xl bg-coral-50 dark:bg-coral-900/30 p-3">
            <p className="text-xs font-semibold text-coral-800 dark:text-coral-200">연 지출</p>
            <p className="mt-1 text-base font-bold text-coral-600 dark:text-coral-200">{formatWon(totalExpense)}</p>
          </div>
          <div className="rounded-xl bg-neutral-50 dark:bg-neutral-950 p-3">
            <p className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">연 잔액</p>
            <p className={`mt-1 text-base font-bold ${totalIncome - totalExpense >= 0 ? 'text-neutral-900 dark:text-neutral-100' : 'text-coral-600 dark:text-coral-200'}`}>
          ...
          {stats.map((m) => (
            <div key={m.month}>
              <div className="flex items-center gap-2 mb-1">
                <span className="w-6 text-xs font-semibold text-neutral-500 dark:text-neutral-400 shrink-0">
                  {m.label}
                </span>
          ...
          <table className="w-full min-w-[420px] text-sm">
            <thead>
              <tr className="border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950">
                <th className="whitespace-nowrap px-4 py-2.5 text-left font-semibold text-neutral-500 dark:text-neutral-400">월</th>
                <th className="whitespace-nowrap px-4 py-2.5 text-right font-semibold text-blue-700 dark:text-blue-300">수입</th>
                <th className="whitespace-nowrap px-4 py-2.5 text-right font-semibold text-coral-600 dark:text-coral-200">지출</th>
                <th className="whitespace-nowrap px-4 py-2.5 text-right font-semibold text-neutral-700 dark:text-neutral-300">잔액</th>
              </tr>
            </thead>
            <tbody>
              {stats.map((m) => {
