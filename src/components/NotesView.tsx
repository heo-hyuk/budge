...
function NotesView({ month }: Props) {
  ...
  async function handleReorderCategories(order: string[]) {
    try {
      setCategories(await reorderNoteCategories(order))
    } catch (err) {
      ...
      <div className="space-y-2 rounded-xl border border-coral-200 dark:border-coral-900 bg-coral-50/40 dark:bg-coral-900/20 p-3">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <ReorderableChipList
            ...
            onTap={(c) => (manageCategories ? handleDeleteCategory(c) : setCategory(c))}
            renderChip={(c, dragging) => (
              <div className="relative">
                <div
                  className={`inline-flex min-h-7 items-center justify-center rounded-full px-2.5 text-xs font-semibold transition-colors ${manageCategories ? 'pr-6' : ''} ${
