...
- 홈 탭과 동일한 날짜별 개별 거래 목록 + 거래마다 "배송완료" 체크박스 — 체크하면
  메모에 "배송완료"가 자동으로 남고(초록색 강조), 목록에서 사라지지 않고
  흐리게/취소선으로만 표시되어 오는 중인 항목과 구분. 체크 해제하면 자동으로
  붙었던 "배송완료" 문구도 함께 제거되어 원래 상태로 되돌아감
...
- 통장 입금을 확인하고 체크하면 해당 거래의 결제 방법이 목표 결제방법으로 즉시
  바뀌며(홈 탭 수입에도 반영) 메모에 "입금완료"가 자동으로 남음(초록색 강조).
  배송 탭과 동일하게 목록에서 사라지지 않고 흐리게/취소선으로만 표시되며,
  체크 해제하면 원래 결제 방법과 메모로 되돌아감(잘못 확인한 경우 되돌리기
  가능 — 원래 결제 방법은 `pending_source_payment_method` 컬럼에 저장해뒀다가
  복원)
- 카드매출 결제방법으로 등록된 수입은 확인 전엔 홈 탭 거래 목록엔 그대로
  보이지만, 잔액/정산(일·주·월·연)/예산/계산기/엑셀 내보내기 등 모든 합산에서는
  완전히 제외됨(확인 전까지는 "미확정 매출"로 취급)
...
- 홈 탭 데스크톱(≥1024px) 레이아웃에서 입력 폼 컬럼에 `sticky`가 걸려있어,
  오른쪽 거래 목록을 끝까지 스크롤해야만 입력 폼이 화면에서 내려가던 문제
  (전형적인 sticky-sidebar 부작용 — 실사용 중 신고로 발견, sticky 제거)
- 배송/카드정산기 체크박스가 이미 완료 문구("배송완료"/"입금완료")가 붙은
  메모에 재차 체크하면 문구를 무조건 또 붙여 중복되던 문제(idempotent
  append로 수정) — 카드정산기를 단방향(체크 즉시 목록에서 제거)에서 배송
  탭과 같은 양방향 토글(되돌리기 가능)로 바꾸는 과정에서 실제 재현됨
