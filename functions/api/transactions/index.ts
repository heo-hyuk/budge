...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, data }) => {
  ...
  const body   = await request.json() as {
    ...
    unsettled?: boolean; is_entertainment?: boolean
  }
  ...
  await env.DB.prepare(
    `INSERT INTO transactions
       (id, type, category, amount, memo, date, merchant, payment_method, card_id,
        original_amount, discount_amount, benefit_id, cashback_amount, unsettled, is_entertainment, user_id, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    ...
    body.cashback_amount ?? 0, body.unsettled ? 1 : 0, body.is_entertainment ? 1 : 0,
    userId, created_at
  ).run()
