...
### 작업 계획
- `TransactionList.tsx`의 날짜별 그룹 내 정렬 기준을 지출 우선 → 수입
  우선으로 변경 (140차에서 지출을 먼저 오도록 바꿨던 것을 반대로 되돌림)

### 수정
- `src/components/TransactionList.tsx` — 그룹 내 정렬 비교자를
  `a.type === 'expense' ? -1 : 1` → `a.type === 'income' ? -1 : 1`로 변경.
  안정 정렬이라 같은 타입 내 시간 순서는 그대로 유지됨

### 완료
- `npm run lint`, `npx tsc -b` 통과

## 2026-08-26 (141차) — 홈 탭 미래 달 이동 허용

사용자 요청: "지금 홈에서 입력하는 달이 현재달이랑 과거만 가능하자나 이거
미래도 가능한게 좋을거 같은데 이기능 추가좀"

### 원인
- `App.tsx` 헤더의 월 이동 "▶" 버튼이 `isCurrentMonth && activeTab !== 'notes'`일
  때 비활성화되어, 메모 탭을 제외한 모든 탭(홈 포함)은 현재 달까지만 이동
  가능했음
- 모바일 스와이프 이동(`handleSwipeEnd`)도 동일하게 현재 달 이후로는
  좌스와이프(다음달)가 막혀 있었음

### 수정
- `src/App.tsx`: 다음 달 버튼 disabled 조건에 `activeTab !== 'home'` 추가 —
  메모 탭처럼 홈 탭도 미래 달 이동 허용(예정된 지출/수입 미리 입력 가능)
- 스와이프 핸들러도 `activeTab === 'home'`일 때 좌스와이프로 다음달 이동
  허용하도록 동일하게 수정
- 예산/정산/계산기 등 나머지 가계부 탭은 기존대로 현재 달까지만 허용 유지

### 완료
- `npm run lint`, `npm run build`(tsc -b 포함) 통과

## 2026-08-25 (140차) — 홈 거래 목록, 같은 날짜 내 지출/수입 분리 정렬

사용자 요청: "홈에서 지출 수입 입력하면 일자별로 나오는데 입력한 순서대로
보여서, 지출이 먼저 나오고 아래쪽에 수입이 나오게 하고 싶다 — 순서는
시간순 유지, 지출/수입만 분리"

### 원인
- `TransactionList.tsx`가 날짜별로 그룹핑만 하고 그룹 내부는 `transactions`
  배열에 들어온 순서(입력/조회 순서) 그대로 렌더링해서, 같은 날 지출과
  수입이 섞여 보임

### 수정
- 날짜별 그룹핑 후 각 그룹을 `type` 기준으로 안정 정렬(stable sort) —
  지출을 앞으로, 수입을 뒤로. `Array.sort`는 ES2019+부터 안정 정렬이
  보장되므로 같은 타입 내 원래 시간 순서는 그대로 유지됨
- `src/components/TransactionList.tsx` 수정 (그룹 생성 직후 정렬 루프 추가)

### 완료
- 코드 수정 완료

## 2026-08-03 (139차) — 수입계산기/지출계산기에 연간 내역 추가

사용자 요청: "수입계산기가 연간내역도 필요해 추가해줘"

### 원인
- `CategoryCalculator.tsx`(수입계산기/지출계산기 공용 컴포넌트)는
  `GET /api/settlement/monthly`만 사용해 항상 선택된 한 달치 일별 표만
  보여줬음 — 연간 합계/월별 내역을 볼 방법이 없었음

### 설계
- 이미 연간 정산 화면(`AnnualSettlementTable.tsx`)이 쓰는
  `GET /api/settlement/annual`(`fetchAnnualSettlement`)이 있어 새 API 없이
  그대로 재사용
- 컴포넌트 안에 "월간/연간" 토글(`periodView` 상태)을 추가 — 헤더의 전역
  월 이동 네비게이션은 그대로 두고, `year = month.slice(0,4)`로 현재 보고
  있는 월이 속한 연도를 자동으로 씀
- 분류 칩 선택(포함/제외)은 기간과 무관하게 공용이라 그대로 재사용,
  일별 표만 연간 모드일 때 "월별 표"(1~12월 행 + 연계)로 바뀌도록 `rows`
  계산을 월간/연간 공통 형태로 추상화

### 완료
- `src/components/CategoryCalculator.tsx` 수정, `tsc -b --noEmit` /
  `oxlint` / `npm run build` 통과
- Playwright로 확인: 수입계산기에서 "급여" 칩 선택 후 "연간" 전환 →
  3월/5월에 등록해둔 급여 300만원씩이 각각 행에, 연계에 600만원으로
  정확히 집계됨을 확인
- 지출계산기(같은 컴포넌트 공유)도 회귀 없이 연간 전환 정상 동작 확인
  (7월 20,000 + 8월 7,200 = 연계 27,200)
- 스키마/백엔드 변경 없음, 프론트 코드 배포만으로 충분

## 2026-08-03 (138차) — 발표용 샘플계정(한가) 1~4월 데이터 정합성 정리

사용자 요청: 137차 완료 보고에서 언급한 "인건비 등 카테고리별 세부 곡선이
완벽히 매끄럽지 않다"는 캐벗에 대해 "니가 전체적으로 문제되는부분은
정리해줘"

### 확인한 문제
- 전 카테고리 월별 합계를 비교한 결과, 137차가 생성한 1~4월 중 실제
  문제는 두 가지로 좁혀짐(나머지 월별 증감은 5~7월 기존 실데이터의
  자연스러운 변동이라 손대지 않음)
  1. 인건비 — 137차가 1~4월에 "매달 10일+25일 2회 지급"으로 생성했는데,
     실제 5월 데이터는 "10일 1회 지급"(96만+101만≈198만원)뿐이고 6월부터
     2회로 늘어난 것 — 그 결과 4월(397만원)이 5월(198만원)보다 오히려
     2배 높아 성장 곡선이 5월에서 뚝 끊겨 보임
  2. 임대료 — 137차가 1~4월엔 아예 생성하지 않아, 5~8월엔 매달
     200만원씩 있는데 1~4월만 0원(정수기 렌탈료 3.3만원도 동일하게 누락)
- `GET /api/settlement/annual?year=2026`으로 카테고리별 월별 합계를
  직접 비교해 위 두 가지 외엔 뚜렷한 부정합이 없음을 확인

### 완료
- [x] 1~4월 인건비 "25일치" 8건 삭제 — 10일 1회 지급만 남겨 1,218,569→
  1,350,967→1,750,288→1,896,111원으로 5월 실데이터(1,976,458원)까지
  자연스럽게 수렴하도록 정리(6월부터 2회 지급으로 실제로 늘어나는
  기존 흐름과도 맞음 — "여름 성수기부터 인력 추가" 서사와 일치)
- [x] 1~4월 임대료 200만원×4건, 정수기 렌탈료 3.3만원×4건 추가 —
  5~8월과 동일하게 매달 고정 반영되도록 함
- [x] `GET /api/settlement/annual?year=2026` 재확인 — 인건비/임대료/
  공과금 모두 1~8월 구간에서 뚜렷한 역전 없이 자연스러운 흐름 확인,
  거래 총건수는 819건으로 변동 없음(삭제 8건 = 추가 8건)
- 생성용 스크립트는 스크래치패드에만 보관, 저장소에는 커밋하지 않음(1회성)
- 코드/스키마 변경 없음, WORKLOG.md만 커밋

---

## 2026-08-03 (137차) — 발표용 샘플계정(한가)에 1~4월 데이터 추가(연간 리포트용)

사용자 요청: 136차 완료 보고 후 "1월부터 있어야 맞아" — 처음엔 계정 가입일
(2026-07-15)에 맞춰 5~7월 중순 데이터를 오히려 정리하려 했으나(가입일 이전
데이터가 있으면 어색하다고 판단), 사용자가 "이건 시연용이니까" 가입일과
데이터 시작일이 맞을 필요 없고 오히려 연간 리포트(12개월 차트) 시연을 위해
1월부터 데이터가 있어야 한다고 정정

### 계획 (코드 변경 없음 — 원격 D1에 API로 직접 시딩)
- 5~8월 기존 데이터는 그대로 두고, 2026-01-01~04-30(120일)만 추가 생성
- 136차와 동일한 방식(요일별 매출 패턴 + 화/금 재료 입고 + 매달 10·25일
  인건비 + 8일 전기/12일 가스/3일 광고비/28일 배달수수료/16일 소모품,
  월요일 정기휴무)을 재사용하되, "1월에 작게 시작해서 여름 성수기까지
  성장" 서사를 위해 월별 성장 배수(1월 0.55 → 4월 0.90, 5월부터는 기존
  실데이터 그대로) 적용
- 재료비 배송 상태는 전부 "배송완료"로 처리(라이브 시연용 미완료 항목은
  이미 8월 구간에 충분히 있음)

### 완료
- [x] 거래 379건 추가(2026-01-01~04-30) — 계정 전체 거래 440→819건,
  날짜 범위 2026-01-01~08-10, 월별 거래일수 결측은 전부 월요일(정기휴무)과
  일치 확인
- [x] `GET /api/settlement/annual?year=2026` 정상 응답(200) — 1~8월 12개월
  차트용 데이터 전부 채워짐 확인
- [x] 월별 손익 확인: 1~4월은 순손실(적자, 성수기 이전 소규모 운영),
  5~6월 흑자 전환, 7~8월(부분월) 다시 소폭 적자 — 매출은 1월 약 650만원
  →7월 약 1,940만원으로 뚜렷한 성장 곡선을 보이나, 인건비 항목만 놓고
  보면 1~4월(월 4회 지급 가정)이 5월 실데이터(월 2회 지급으로 추정)보다
  오히려 높게 나오는 등 카테고리별 세부 곡선까지 완벽히 매끄럽진 않음 —
  전체 매출/손익 그래프로 보는 발표용으로는 문제없지만, 카테고리별 월별
  상세까지 파고들면 어색할 수 있어 필요시 추가 다듬음 가능하다고 사용자에게
  안내함
- 생성용 스크립트는 스크래치패드에만 보관, 저장소에는 커밋하지 않음(1회성)
- 코드/스키마 변경 없음, WORKLOG.md만 커밋
...
## 2026-08-03 (136차) — 발표용 샘플계정(한가) 데이터를 8/10 발표일 기준으로 갱신

사용자 요청: "한가계정이 발표할수 있게 8월 10까지 데이터 세팅 해줘 모든 기능
구현가능하고 문제 없도록" — 99차/104~107차에서 만든 발표용 계정(한가,
`aaa@aaa`, user_id `8daf2fba-...`)은 2026-07-27 발표를 기준으로 세팅돼 있었고
이후 거래가 08-01(임대료 자동생성) 1건뿐 — 다음 발표일(8/10)까지 공백이 있고,
카드 혜택/예산/메모/빠른입력템플릿 등 아직 한 번도 시연 데이터가 없던 기능도
있어 전체 기능을 빠짐없이 시연 가능하도록 데이터를 보강

### 확인한 현재 상태 (원격 D1, 한가 계정)
- 거래 388건(2026-05-01~08-01), 카드 1개(사업자카드), 카드 혜택/혜택그룹 0건,
  예산 0건, 메모 0건, 빠른입력템플릿 0건 — 정산/거래입력/고정지출/카드정산기/
  수입지출계산기/세금계산기는 이미 시연 가능한 상태였지만 카드 혜택 매칭·
  예산 관리·메모장·빠른입력·체크카드(즉시결제) 토글은 시연 데이터가 없었음
- 106차에서 만든 카드정산기용 "미확인 매출" 3건(2026-07-27, 배민/요기요/
  쿠팡이츠)이 발표일 기준으로는 2주 넘게 지난 상태라 그대로 두면 부자연스러움

### 계획 (코드 변경 없음 — 원격 D1에 API로 직접 시딩)
- 스크래치패드에 Node 스크립트(`seed_aug10.mjs`, 저장소 미커밋) 작성, 실제
  운영 API(`https://budget-3wb.pages.dev`)에 세션 로그인 후 호출하는 방식으로
  진행 — 서버 검증/파생 로직(혜택 매칭은 클라이언트가 계산해 보내는 구조 등)을
  그대로 통과시켜 화면과 100% 같은 데이터 형태가 되도록 함
- 2026-07-27 미확인 매출 3건은 확인 처리(되돌리기 데모 풀에 편입)하고,
  카드정산기용 새 미확인 매출 3건을 발표 직전 영업일(08-09, 일요일)에 추가
- 신규 카드 2개 추가: "개인카드(롯데 LOCA LIKIT)" — 카드 상품 프리셋 그대로
  혜택그룹 1개+혜택 5개 등록(배달앱 5% 할인 포함), "지역화폐카드" —
  체크카드(즉시결제) 토글 시연용
- 2026-07-28~08-10(14일) 매출/지출 거래를 기존 3개월치 데이터의 요일별
  평균(월요일 정기휴무, 금·토·일 매출 증가, 화·금 생닭 입고 등)에 맞춰 생성,
  그중 일부에 카드 혜택 자동매칭(원금/할인 전후 금액), 접대성 지출
  (`is_entertainment`), 비정산(`unsettled`) 토글, 배송 미완료 상태를 섞어
  각 기능이 실제 데이터로 시연되도록 구성
- 예산 4건(재료비/인건비/광고비/공과금, 매월 반복) — 인건비는 8/10 1차
  급여만으로 이미 초과(101%)하도록, 광고비는 근접(89%)하도록 의도적으로
  구성해 "예산 초과 배너"를 실제로 시연 가능하게 함
- 빠른입력템플릿 4건, 메모 10건(2026-05~08, 5개 분류 고루 사용) 추가

### 완료
- [x] 카드 2개 추가(개인카드-롯데LOCA LIKIT 프리셋/지역화폐카드-체크카드),
  혜택그룹 1개 + 혜택 5개 등록 확인
- [x] 2026-07-27 미확인 매출 3건 확인 처리, 08-09에 새 미확인 매출 3건(배민/
  요기요/쿠팡이츠) 추가 — 카드정산기 대기 목록 갱신 확인
- [x] 2026-07-28~08-10 거래 44건 추가(거래 총 388→440건), 그중 카드 혜택
  자동매칭 2건(원금→할인가), 접대성 지출 1건, 비정산 1건, 배송 미완료
  재료비 3건 생성 확인
- [x] 예산 4건 등록 — `GET /api/budgets` 응답으로 재료비 25%/인건비
  101%(초과)/광고비 89%/공과금 46% 정상 계산 확인
- [x] 빠른입력템플릿 4건, 메모 10건 등록 확인(각각 개수 조회로 검증)
- [x] `GET /api/tax/estimate?month=2026-08`, `GET /api/settlement/monthly
  ?month=2026-08&basis=billing|transaction` 모두 정상 응답(200) 확인 —
  출금일 기준에서 카드 거래가 청구일(08-25)로 올바르게 이동, 체크카드
  (즉시결제)는 거래일 그대로 유지되는 것도 함께 확인
- 생성용 스크립트는 스크래치패드에만 보관, 저장소에는 커밋하지 않음(1회성)
- 코드/스키마 변경 없음, WORKLOG.md만 커밋

---

## 2026-08-01 (135차) — 엑셀 정산표를 수입/지출 시트로 분리(인쇄 가독성)

사용자 요청: "이거 지금 인쇄 해보니까 너무 보기가 힘들어서 수입이랑 지출을
나눠야 할거같은데 지금 정산표를 나눠서 엑셀파일로 보여줘야 될거같아" —
134차에서 만든 "정산표" 시트가 수입 3분류+수입합계+지출 9분류+지출합계를
한 표(14열)에 다 넣어서, 인쇄하면 폭이 너무 넓어 읽기 어려웠음

### 완료
- `buildSettlementGridSheet`에 `type: 'income'|'expense'` 파라미터를 추가해
  수입표/지출표를 각각 별도로 생성하도록 리팩터링
- "정산표" 단일 시트 대신 "정산표(수입)"(날짜+수입분류 3개+수입합계, 5열)
  / "정산표(지출)"(날짜+지출분류 9개+지출합계, 10열) 2개 시트로 분리 —
  거래내역 → 정산표(수입) → 정산표(지출) → 월별요약 → 카드별정산 순
- 인쇄 여백을 좁게 설정(`!margins`)해 조금이라도 더 종이 폭에 여유 있게
  들어가도록 함(SheetJS 커뮤니티 빌드는 인쇄 방향/페이지 맞춤(`!printSetup`)
  자체를 지원하지 않아 그 부분은 반영 못함 — 필요하면 엑셀에서 인쇄 시
  "가로 방향"으로 직접 설정 안내가 필요할 수 있음)
- 모달 안내 문구를 "5개 시트로 내보냅니다"로 갱신
- `tsc -b --noEmit` / `oxlint` / `npm run build` 통과
- Playwright + xlsx 파싱으로 다운로드 파일 검증: "정산표(수입)"엔 급여/
  용돈/기타수입 3열, "정산표(지출)"엔 식비~기타 9열만 각각 들어있고,
  8/1 거래(출금일 기준 청구일 25일로 집계)가 정산표(지출)의 25일 행에
  정확히 7,200원으로 반영됨을 확인

## 2026-08-01 (134차) — 엑셀에 화면과 동일한 "정산표"(일별×분류 그리드) 시트 추가, 출금일 기준 지원

사용자 요청: "정산표가 화면에 보이는형태로 엑셀에 출력되는개 필요한데 그리고
출금일 기준으로 해서 가능할까?" — 133차에서 추가한 엑셀 내보내기의
"거래내역"/"월별요약" 시트는 각각 거래 단위 상세와 월 합계뿐이라, 화면의
"분류별 표"(날짜×분류 그리드)와 같은 모양이 아니었음

### 설계
- 화면의 "분류별 표"는 클라이언트에서 직접 재계산하지 않고 서버
  `GET /api/settlement/monthly?month=&basis=`(129차에서 만든 API)를 그대로
  사용 — 엑셀도 같은 데이터를 그대로 재사용하면 화면과 100% 동일한 결과가
  보장됨(특히 "출금일 기준"에서 카드 거래를 청구일 하루로 몰아 담는 로직을
  클라이언트에서 다시 구현하면 어긋날 위험이 있어, 직접 계산하지 않고
  기존 계산을 재사용하는 쪽을 선택)
- 내보내기 시 이미 가져온 거래 목록에서 걸쳐 있는 월(YYYY-MM) 목록을 뽑아
  각 월에 대해 `fetchMonthlySettlement(month, basis)`를 병렬 호출, 그
  결과로 "정산표" 시트를 만듦(월 여러 개면 시트 하나에 월별로 이어붙임)
- 기준(출금일/거래일)은 사용자가 별도로 다시 고르게 하지 않고, 화면에서
  이미 토글해둔 현재 설정(`getMonthlyBasis()`, 서버에 저장돼 화면/엑셀
  공통으로 쓰임)을 그대로 사용 — "화면에 보이는 형태 그대로"라는 요청과
  가장 잘 맞음
- 정산표 조회가 실패해도(네트워크 오류 등) 나머지 3개 시트는 정상
  내보내지도록 별도 try/catch로 격리

### 완료
- `src/lib/exportExcel.ts`에 `buildSettlementGridSheet` 추가(화면의
  `MonthlySettlementTable.tsx`와 동일한 날짜×분류 헤더/합계 구성), 시트
  순서를 거래내역 → 정산표 → 월별요약 → 카드별정산으로 정리
- `src/components/ExportButton.tsx`가 내보내기 시 월별 정산 데이터를 함께
  가져와 전달하도록 수정, 안내 문구에 현재 기준(출금일/거래일)을 노출
- `tsc -b --noEmit` / `oxlint` / `npm run build` 통과
- Playwright로 실제 다운로드 검증: 월간 화면에서 "출금일 기준"으로 전환한
  뒤 내보내기 → 모달에 "정산표(화면과 동일, 출금일 기준)" 문구 확인,
  다운로드한 xlsx의 "정산표" 시트에서 마감14·결제25 카드 거래(원래 8/1
  거래일)가 화면과 동일하게 25일 행에 7,200원으로 집계됨을 확인(거래일이
  아닌 청구일 기준으로 정확히 이동)

## 2026-08-01 (133차) — 엑셀 내보내기를 월간 정산 화면에도 추가

사용자 요청: "엑셀파일 추출하는게 연간에서만 되는데 월별로도 필요해 상세
기록까지 다 나올수 있게"

### 원인
- `ExportButton.tsx`는 이미 기간(preset)을 완전히 파라미터화해 만들어져
  있었고 `month` prop까지 받게 설계돼 있었지만(`defaultPreset="this_month"`
  가 기본값), 실제로 화면에 배치된 곳은 `AnnualReport.tsx`(연간 → 차트·
  내보내기)와 `SearchView.tsx`(검색, 전체 기간) 두 곳뿐이었음 — 월간 정산
  화면(`OverviewView.tsx`)에는 버튼 자체가 없었던 것
- `exportTransactionsToExcel`이 만드는 "거래내역" 시트는 원래도 날짜/구분/
  분류/구매처/결제방법/카드명/원금액/할인액/실결제액/적립예정액/메모까지
  거래 단위 상세 행을 전부 포함하고 있었음 — "상세 기록"은 이미 구현돼
  있었고 버튼이 월간 화면에 없었던 게 문제

### 완료
- `OverviewView.tsx`의 월간 뷰 헤더(분류별 표/카드별 청구 토글 옆)에
  `<ExportButton defaultPreset="this_month" month={month} />` 추가 —
  AnnualReport와 동일한 컴포넌트를 그대로 재사용, 새 코드 없이 배치만 추가
- `tsc -b --noEmit` / `oxlint` / `npm run build` 통과
- Playwright로 실제 다운로드까지 검증: 2026년 8월 화면에서 "엑셀
  내보내기" → 기본값 "8월"(2026-08 기간) 선택된 상태로 모달이 뜨고,
  다운로드한 xlsx의 "거래내역" 시트에 8월 거래(원금액 8,000/할인액
  800/실결제액 7,200 등 상세 컬럼 전부)만 정확히 포함됨을 확인.
  "월별요약"·"카드별정산" 시트도 함께 정상 포함됨
- 스키마/백엔드 변경 없음, 프론트 코드 배포만으로 충분

## 2026-08-01 (132차) — 저장된 거래 목록에도 할인 전/후 금액 함께 표시

사용자 요청: 131차 확인 후 "수정은 확인했어 근데 입력금액이랑 계산금액
보여주는건 어떻게 된거야?" — 130차에서 만든 "원래금액 → 실결제금액" 표시는
입력/수정 폼의 실시간 미리보기 박스에만 있었고, 저장된 거래를 목록에서
그냥 볼 때(수정 모드가 아닐 때)는 할인 후 최종 금액만 보이고 원래 얼마였는지
전혀 알 수 없었음 — 이 부분이 빠졌던 것

### 원인
- `TransactionList.tsx`의 "일반 표시"(비수정 상태) `<li>`는 `tx.amount`
  (할인 후 최종 금액)만 렌더링하고, `tx.original_amount`/`discount_amount`/
  `cashback_amount`는 어디서도 표시하지 않았음

### 완료
- 금액 표시를 두 줄로 변경: `discount_amount > 0`이면 최종 금액 위에
  취소선 처리된 원래 금액(`original_amount`)을 작게 함께 표시
  (예: ~~8,000원~~ / -7,200원)
- `cashback_amount > 0`인 거래는 결제 금액 자체는 안 깎이므로, 비정산
  뱃지와 같은 자리에 파란색 "적립 {금액}" 뱃지를 추가로 표시
- `tsc -b --noEmit` / `oxlint` / `npm run build` 통과
- Playwright로 확인: 8,000원 → 7,200원(800원 할인) 거래가 홈 화면 최근
  내역에 "8,000원(취소선)"과 "-7,200원"이 함께 표시됨을 확인
- 스코프: 거래를 나열하는 화면이 여러 개(일일/주간 정산, 카드별 청구,
  검색, 배송 등) 있지만 이번엔 사용자가 실제로 확인한 홈 화면 목록
  (TransactionList.tsx)만 수정 — 다른 화면도 필요하면 후속 요청으로 처리

## 2026-08-01 (131차) — 홈 화면 인라인 수정에도 카드 혜택 선택 적용

사용자 요청: 130차에서 "TransactionList의 더 단순한 인라인 수정 폼은 혜택 UI
자체가 없어 범위에서 제외"했다고 보고했더니 "필요해" — 홈 화면 최근 내역의
"수정" 버튼(TransactionList.tsx 인라인 수정)에도 동일하게 적용해달라는 요청

### 설계
- 130차에서 `TransactionForm.tsx`에 적용한 것과 완전히 동일한 패턴을
  `TransactionList.tsx`의 인라인 수정 폼에도 이식: 혜택 매칭 상태(matches/
  selectedMatch/matchLoading/debounce), `startEdit` 시 `original_amount`
  우선 채우기, `handleSave`에서 할인/적립 필드 계산해 전달, "혜택 자동
  적용: 이름 / 원래금액 → 실결제금액 (할인액)" 박스 + 복수 매칭 시 라디오
  선택 UI
- 별도 컴포넌트 분리 없이 TransactionForm의 로직을 그대로 복제 — 두 폼의
  나머지 구조(분류/날짜/메모 입력 등)가 이미 상당히 다르게 구성돼 있어
  공통 훅으로 추출하는 것보다 복제가 더 단순하고 안전하다고 판단

### 완료
- `src/components/TransactionList.tsx` 수정, `tsc -b --noEmit` / `oxlint` /
  `npm run build` 통과
- Playwright + curl로 검증(포트 8799): 카드 혜택 10% 할인이 걸린 5,000원
  거래(실결제 4,500원)를 홈 화면 최근 내역에서 "수정" 클릭 → 금액
  입력창에 5,000원이 채워지고 "혜택 자동 적용: 식비10% / 5,000원 → 4,500원
  (500원 할인)" 박스가 정상 표시됨을 확인
  - 금액을 8,000원으로 바꾸면 실시간으로 "8,000원 → 7,200원 (800원 할인)"로
    재계산되고, 저장 후 DB에 amount=7200/original_amount=8000/
    discount_amount=800/benefit_id 모두 정확히 반영됨을 확인
- 코드 변경만 있어 원격 배포는 코드 배포로 충분(스키마 변경 없음, 130차에서
  이미 백엔드 필드는 추가됨)
...
실제 계산이랑 비교 한번 해보자" — `functions/lib/tax.ts`(1인 사업자 부가세/종합소득세
추정)가 배포는 됐지만 실거래 데이터로 API 응답을 수기 계산과 대조 검증한 적은 없었음

### 방법
- 로컬 D1(`wrangler d1 execute --local`)에 전용 테스트 유저(`taxtest-user`) +
  사업용카드/개인카드/현금 결제 + 경비제외 분류(개인비용) + 접대비 거래 +
  비정산(`unsettled=1`)/카드정산기 미확인(`예정`) 거래를 섞어 1~3월 데이터 삽입
- `wrangler pages dev dist --port=8788`로 로컬 풀스택 서버 실행(세션 쿠키 직접
  DB에 삽입해 로그인 우회), `GET /api/tax/estimate?month=...`를 실제 호출
- 같은 데이터로 부가세/종소세 계산식을 손으로 계산해 API 응답과 대조
  - 주의: `wrangler pages dev`에 `--d1=DB=budget-db` 플래그를 주면 wrangler.toml의
    `database_id`를 무시하고 별도의 로컬 D1 파일(빈 스키마)이 생성됨 — 플래그 없이
    wrangler.toml 설정만으로 띄워야 `wrangler d1 execute`가 쓰는 것과 동일한 D1 파일에 연결됨

### 결과 — 전부 API 응답 = 수기계산 일치
| 시나리오 | est_vat | est_income_tax | tax_reserve_fund | real_net_income |
|---|---|---|---|---|
| 일반과세자 (3월, 누적3개월) | 350,000 | 891,000 | 1,241,000 | 2,109,000 |
| 간이과세자 20% (3월) | 100,000 | 891,000 | 991,000 | 2,359,000 |
| 프리랜서 3.3% (3월) | null | 891,000 | 891,000 | 2,459,000 |
| 간이과세자, 부가율 미입력 | null(vat_calculable=false) | 891,000 | 891,000 | 2,459,000 |
| 일반과세자 (1월, 누적1개월) | 200,000 | 140,250 | 340,250 | 1,209,750 |
| 일반과세자 (2월, 누적2개월) | 280,000 | 453,750 | 733,750 | 1,866,250 |

검증된 로직: 부가세(일반 10% / 간이 부가율×10% / 프리랜서 해당없음), 종소세 누진공제
연환산 후 경과월 일할계산, 필요경비 opt-out 분류 제외, 부가세 매입세액공제 조건
(사업용카드 + 접대비 아님), 비정산·카드정산기 미확인 거래 집계 제외 — 전부 의도대로 동작

에러 케이스: 세율 미등록 연도(2025) → 400 "2025년 세율 정보가 아직 등록되지 않았습니다",
잘못된 month 포맷 → 400 "월 형식이 올바르지 않습니다" — 둘 다 정상 동작 확인

코드 변경 없음 — 로컬 D1 테스트 데이터/세션은 검증 후 전부 삭제, dev 서버 프로세스 종료
...
진행. 기존 로그인/가입 폼(`AuthPage.tsx`)은 그대로 두고 그 위에
마케팅 섹션을 추가하는 방식으로 결정(기존 검증된 폼 로직 손대지 않음)

### 계획
- 데모 계정에 그럴듯한 샘플 데이터(수입/지출, 카드+혜택, 예산, 세금
  설정) 채운 뒤 모바일 뷰포트로 홈/카드/세금계산기 화면 실제 스크린샷
  캡처 → `public/screenshots/`에 저장(목업 아닌 실제 렌더링)
- `src/components/LandingIntro.tsx`(신규) — 헤드라인 + 핵심 기능
  4~5개 요약(아이콘 없이 텍스트 위주, 기존 세금계산기 화면 톤과
  일관) + 스크린샷 섹션 + "무료로 시작하기" CTA(로그인 폼으로 스크롤)
- `src/components/AuthPage.tsx` — 상단에 `LandingIntro` 추가, 기존
  로그인/가입 카드는 그대로 아래로 옮기고 `id="signup"` 앵커 부여
  (좁은 카드 레이아웃과 폼 로직 자체는 변경 없음)
- `index.html`의 `og:image`를 앱 아이콘 대신 실제 스크린샷으로 교체 검토
- `wrangler pages dev` + Playwright로 실제 렌더링/스크롤 이동/모바일
  레이아웃 확인
...
### 완료
- 계획한 대로 `workers/monthly-tax-reporter/`(신규: `package.json`,
  `tsconfig.json`, `wrangler.toml`, `src/tax.ts`, `src/index.ts`) 전부
  작성, `npm install` + `tsc --noEmit` 통과. 루트 `oxlint`/`tsc -b`도
  기존과 동일하게 통과(워커는 별도 프로젝트라 영향 없음)
- README에 "세금 마감 리포트 Push 알림" 기능 설명, 디렉토리 트리,
  "Cron Worker (세금 마감 리포트) 배포" 섹션 추가
- `wrangler dev --test-scheduled`로 로컬 D1(이 워커 전용, `schema.sql`로
  초기화)에 테스트 유저 2명(세금설정 있음/없음), 거래, 사업용 카드,
  가짜 P-256 구독(엔드포인트를 실제 410을 반환하는 `httpbin.org/status/410`
  로 잡아 만료 정리까지 실제로 검증), 로컬 전용 VAPID 테스트 키쌍
  (`.dev.vars`, gitignore 대상)을 넣고 `/__scheduled`로 실제 시나리오 재현:
  - 세금 설정 있는 유저 → `notification_log`에 `type='monthly_tax_report'`,
    `year_month`가 트리거 시점(7월) 기준 정확히 전달(6월)로 기록됨을 확인
  - 같은 달 재실행 시 중복 기록 안 됨(dedup) 확인
  - 세금 설정 없는 유저 → 에러 없이 조용히 스킵, `notification_log`
    기록 없음 확인
  - 발송 응답 410 → 해당 구독이 `push_subscriptions`에서 실제로 삭제됨을
    확인(진짜 410을 반환하는 외부 엔드포인트로 검증)
  - 메시지 본문을 임시 로그로 직접 확인: 일반과세자(부가세 계산 가능)는
    "예상 순수익 2,085,000원 ... 세금 예비비 415,000원 ... 추천해요
    (추정치, 실제와 다를 수 있어요)", 프리랜서 3.3%(부가세 해당 없음)는
    "예상 순수익 2,335,000원 ... 부가세는 마이페이지에서 정보를
    입력하면 함께 안내해드려요"로 요청 사항과 정확히 일치. 두 금액 모두
    수기 계산과 일치(부가세 250,000 + 종소세 165,000 = 415,000 등)
  - "매월 말일 자정에만 동작" 조건은 실제 날짜(7월 28일)가 1일이 아니라
    자연 발생 검증이 불가해, 로컬 검증 중에만 그 조건을 임시로 우회하고
    검증 직후 원래 코드로 정확히 원복(diff 없음 확인) — 발송/dedup/스킵/
    만료정리/본문 분기 등 나머지 로직은 전부 실제 조건 그대로 검증함
  - 테스트로 만든 임시 디버그 로그(console.log)도 검증 후 제거
  - 테스트 데이터는 이 워커 전용 로컬 D1에서 정리, `.dev.vars`와
    `.wrangler`(둘 다 gitignore 대상) 삭제, 프로세스 종료
- 원격 배포는 진행하지 않음 — `wrangler secret put VAPID_PRIVATE_KEY` +
  `wrangler deploy`는 사용자 확인 후 진행 예정(README에 절차 기록)

## 2026-07-28 (117차) — "세금 계산기" 화면(수입/지출 계산기 옆 탭)

사용자 요청: 116차 `/api/tax/estimate`를 그대로 보여주는 "세금 계산기"
탭을 수입계산기/지출계산기 옆에 추가. 코랄+카드형 톤, 아이콘 없이(카드
내부), 세후 예상 순수익 강조+"추정치" 배지를 숫자 바로 옆에, 총매출→
필요경비→세금 예비비 단계별 카드, 부가세/종합소득세 카드, 하단 면책
조항(눈에 띄게), "수입/지출 계산기와 기준이 다르다" 안내 한 줄

### 계획
- `src/types.ts` — `TaxEstimate` 프론트 타입(백엔드 응답 그대로)
- `src/lib/api.ts` — `fetchTaxEstimate(month)`
- `src/components/TaxCalculatorView.tsx`(신규) — 로딩/에러/데이터 3상태.
  에러 메시지에 "세금 설정"이 포함되면 마이페이지 바로가기 버튼도 함께
  표시. vat_not_applicable/vat_calculable 조합별로 부가세 카드 내용 분기
- `src/App.tsx` — `taxCalculator` 탭 추가('지출계산기' 바로 옆),
  월 네비게이션 표시 조건에도 추가, 렌더 분기 추가(onOpenMyPage로
  MyPage 모달 오픈 연결)
- `wrangler pages dev` + Playwright로 실제 화면 확인: 일반과세자/
  간이과세자(부가율 있음/없음)/프리랜서 각 케이스 별 부가세 카드 표시,
  세금 설정 미저장 시 에러+마이페이지 이동 버튼, 추정치 배지가 숫자
  옆에 붙어 보이는지 스크린샷으로 확인

### 완료
- 계획한 4개 파일(`src/types.ts`, `src/lib/api.ts`,
  `src/components/TaxCalculatorView.tsx`, `src/App.tsx`) 모두 작성 완료
- `tsc -b --noEmit` / `oxlint` / `npm run build` 모두 통과
- `wrangler pages dev` + Playwright로 실제 화면 검증(뷰포트를 넉넉히
  키워 스크롤 이슈 없이 진행):
  - 세금 설정 미저장 상태 → 에러 문구 + "마이페이지로 이동" 버튼 노출,
    클릭 시 실제로 마이페이지가 열리고 "사업자 세금 설정" 섹션이
    보이는지 확인
  - 일반과세자 시나리오(확정매출 200만 + 예정매출 50만 등록) →
    "세후 예상 순수익" 큰 숫자 바로 옆에 "추정치" 배지가 항상 붙어
    보임(하단 아님) 확인. 단계별 계산 카드(총매출 2,000,000 →
    −필요경비 400,000 → −세금예비비 275,600 → =1,324,400)가 부가세
    170,000 + 종소세 105,600 수기 계산과 정확히 일치. 부가세 카드에
    매입세액공제 안내 문구, 하단 면책 조항(작은 회색 글씨 아닌 amber
    박스로 눈에 띄게), 상단 "수입/지출 계산기와 기준이 다르다" 안내
    모두 표시 확인
  - 간이과세자(부가율 미입력) → "부가율 미입력" 안내 + 마이페이지
    이동 버튼 확인
  - 프리랜서(3.3%) → "부가세 해당 없음" 안내 확인
  - 세 케이스 모두 세금 예비비/실질 순수익 숫자가 부가세 유무에 따라
    올바르게 재계산됨을 확인
...
→ 조사 결과 메모(`notes`) 첨부 이미지는 R2에 원본 그대로 저장되고
있었고(5MB 상한 검사만 존재, 압축/리사이징 로직 전무), "클라이언트
리사이즈+WebP 변환" 방식으로 진행하기로 확정

### 계획
- `src/components/NotesView.tsx`의 `handleImageSelect`에서 파일 선택
  직후 `<canvas>`에 그려 긴 변을 특정 크기(예: 1600px) 이하로 축소한 뒤
  `canvas.toBlob(..., 'image/webp', quality)`로 재인코딩해 압축된
  Blob을 `imageFile` state에 담아 업로드하도록 수정
- 압축 로직은 재사용 가능하도록 별도 유틸 함수로 분리 검토
  (`src/lib/imageCompress.ts` 등)
- 서버(`functions/lib/noteImages.ts`)의 5MB 상한, R2 저장 구조,
  허용 타입 목록은 변경하지 않음 — 압축은 업로드 전 프론트 단계에서만
  적용
- WebP를 지원하지 않는 예외적 환경(구형 브라우저 등)을 고려해
  `canvas.toBlob`이 실패/미지원일 경우 원본 파일 그대로 폴백
- `wrangler pages dev` + Playwright로 메모 작성 화면에서 이미지 첨부 시
  실제 업로드되는 파일 용량이 줄어드는지, 미리보기/저장/조회가 정상
  동작하는지 검증
...
사용자 요청: "카드 탭에 체크카드 토글을 하나 만들어서 혜택은 적용되고
결제는 바로 적용 되도록 하는건 어때 요새 지역 화폐 카드가 많이있자나"
(대전사랑카드 등 지역화폐 캐시백 자동 검색/적용 건은 "우선 체크카드
기능만 넣자 혜택은 애매모호하다"는 판단에 따라 이번 범위에서 제외)

### 설계
- 이 앱의 "카드"는 신용카드(청구일/마감일이 있고 나중에 청구되는
  구조) 개념으로 설계돼 있어 체크카드(결제 즉시 통장에서 차감)와는
  근본적으로 안 맞음 — 카드마다 "체크카드(즉시결제)" 여부를 토글할 수
  있는 `is_debit` 플래그를 추가해 이 카드만 청구주기 계산을 건너뛰고
  거래일 그대로 즉시 반영되게 함
- 혜택(할인/적립) 매칭은 이미 `card_id` 등록 여부만으로 판단해서
  `is_debit`과 무관하게 그대로 동작 — 별도 수정 불필요(요청하신
  "혜택은 적용되고" 요구사항을 이미 만족)
- "카드 정산 알림"(청구 마감일 푸시)은 확인해보니 아직 실제 발송
  로직(크론 등)이 붙어있지 않아 이번 변경과 무관 — 나중에 실제로
  붙일 때 이 플래그로 체크카드는 건너뛰도록 하면 됨(지금은 손댈 것 없음)
- 체크카드로 표시된 카드는 `functions/api/cards`가 여전히 billing_day/
  closing_day 값을 요구(스키마 NOT NULL)하니 폼에서 입력 UI만 숨기고
  기존 기본값(25/14)을 그대로 실어 보냄 — 실제로는 안 쓰이는 값이라
  문제 없음

### 계획
- `migrations/029_add_card_is_debit.sql` + `schema.sql` 동기화 —
  `cards.is_debit` 컬럼 추가(기본값 0)
- `functions/api/cards/index.ts`(POST), `functions/api/cards/[id].ts`
  (PATCH) — `is_debit` 필드 추가
- `src/types.ts` — `Card.is_debit`, `NewCard.is_debit?` 추가
- `src/components/CardManager.tsx` — 폼에 "체크카드(즉시결제)" 토글
  추가(켜면 결제일/마감일 입력·말일 토글·청구기간 미리보기 섹션을
  전부 숨기고 대신 안내 문구 표시), 카드 목록 헤더에도 "체크카드"
  라벨 표시, `startAdd`/`startEdit`/`handleSave`에 `is_debit` 반영
- `src/components/MonthlyReport.tsx` — "출금일 기준" 모드에서도
  `card.is_debit`인 카드는 `getCardBillingPeriod` 대신 그 달 거래를
  그대로 합산(거래일 기준과 동일하게), 청구기간 라벨도 "즉시결제"로 표시
- `src/lib/exportExcel.ts`의 "카드별정산" 시트도 동일하게 체크카드는
  청구기간 대신 월 전체를 그대로 합산
- 로컬 D1에 마이그레이션 적용 후 `wrangler pages dev` + Playwright로
  체크카드 토글 시 결제일/마감일 UI가 숨는지, 체크카드로 등록한
  거래가 월간 리포트에서 청구기간 이동 없이 즉시 그 달에 반영되는지,
  혜택 매칭은 체크카드에서도 정상 동작하는지 검증
- 원격(production) D1 마이그레이션은 로컬 검증 완료 후 사용자에게
  확인받고 진행
...
### 원인
- `App.tsx`의 월 네비게이션 "▶"(다음 달) 버튼이 `disabled={isCurrentMonth}`로
  탭 구분 없이 전역적으로 항상 현재 달에서 막혀 있었음. 홈/예산/비정산/
  계산기/배송/카드정산기 같은 가계부 성격 탭은 미래 달에 거래가 있을
  수 없으니 합리적인 제약이지만, 메모(일기 성격) 탭은 앞으로의 일정·
  약속 등을 미리 적어두는 용도로도 쓰이므로 이 제약이 맞지 않음
- 백엔드(`GET /api/notes?month=`)나 `NotesView.tsx`의 날짜 생성 로직은
  애초에 미래 달을 막지 않아 이 버튼 조건만 고치면 됨(다른 코드 변경 불필요)

### 계획
- `src/App.tsx` — "▶" 버튼의 `disabled` 조건을
  `isCurrentMonth && activeTab !== 'notes'`로 변경해 메모 탭에서만
  다음 달로 자유롭게 이동 가능하게 함
- `wrangler pages dev` + Playwright로 메모 탭에서 다음 달 이동이
  되는지, 홈/예산 등 다른 탭에서는 기존처럼 현재 달 이후로 못 넘어가는지
  (회귀 없는지) 검증

### 완료
- [x] `src/App.tsx` — "▶" 버튼 `disabled` 조건을
  `isCurrentMonth && activeTab !== 'notes'`로 변경
- [x] `npx tsc -b --noEmit`, `npm run lint` 모두 통과
- [x] `wrangler pages dev` + Playwright로 검증: 홈/예산 탭은 기존처럼
  현재 달에서 다음 달 버튼이 비활성화됨(회귀 없음), 메모 탭에서는
  버튼이 활성화되어 "2026년 7월 → 2026년 8월"로 정상 이동됨을 확인,
  다음 달로 이동한 상태에서 메모 추가 폼이 정상적으로 열리고 저장도
  성공함을 확인
...
### 완료
- [x] `src/lib/memoHighlight.tsx` — `appendDoneTag`/`stripDoneTag`/
  `hasDoneTag` 헬퍼 추가(날짜 있는/없는 형식 모두 인식하는 정규식),
  `renderMemoWithHighlights`가 "배송완료"/"입금완료" 뒤에 붙는
  "(YYYY-MM-DD)"까지 함께 초록색으로 강조하도록 확장
- [x] `src/components/DeliveryView.tsx` — `handleToggleDelivery`를
  새 헬퍼 기반으로 교체(중복 문자열 비교 로직 제거), 체크 시
  "배송완료(오늘 날짜)" 삽입
- [x] `src/components/CardSettlementView.tsx` — `handleToggleSettlement`도
  동일하게 교체, 확인 시 "입금완료(오늘 날짜)" 삽입, 되돌리기 시 정확히 제거
- [x] `npx tsc -b --noEmit`, `npm run lint` 모두 통과
- [x] `wrangler pages dev` + Playwright로 검증: 배송 탭에서 체크하면
  메모에 "배송완료(2026-07-27)"이 초록색 굵은 글씨로 표시됨(날짜까지
  포함해 강조됨), 체크 해제 시 정확히 제거됨. 카드정산기에서도
  "예정"→"계좌이체" 확인 시 "입금완료(2026-07-27)"이 정확히 붙고,
  되돌리기(체크 해제) 시 정확히 제거됨을 확인
  - 검증 중 테스트 스크립트가 카드정산기 탭 상단 안내 문구에 예시로
    적힌 "입금완료"라는 단어를 실제 거래 메모로 착각해 오탐하는
    문제가 있었음 — 검색 범위를 `<li>`(실제 거래 목록 항목)로 좁혀
    재검증 완료(앱 코드 문제 아님, 테스트 스크립트만 수정)
- 미완료 항목 없음(스키마/마이그레이션 변경 없음)
...
### 완료
- [x] `README.md` 상단 요약 통계 갱신 — 2026-07-14~2026-07-23/93차/197커밋/
  fix 36건 → 2026-07-14~2026-07-27/107차/225커밋/fix 41건
- [x] "배송" 절 — 체크 시 메모에 "배송완료" 자동 추가(초록 강조), 체크
  해제 시 자동 제거되는 동작 반영
- [x] "카드 정산기" 절 — 기존 단방향(체크 즉시 목록에서 제거) 설명을
  배송 탭과 동일한 양방향 토글(체크 해제로 되돌리기 가능,
  `pending_source_payment_method`에 원래 결제방법 저장) 방식으로 전면
  수정
- [x] DB 스키마 절 — `transactions` 컬럼 목록에
  `pending_source_payment_method` 추가 + 설명 문단 추가(migration 028)
- [x] "개발 / 검증 프로세스" 버그 목록에 이번 세션에서 실사용 중 발견된
  두 건 추가 — 홈 탭 데스크톱 sticky 레이아웃 버그(105차), 완료 문구
  중복 추가 버그(104차)
- 오늘(103~107차) 작업 전부 완료 및 배포 완료(코드 변경분은 GitHub
  Actions로 자동배포, 데이터 가공은 원격 D1에 직접 반영), 미완료 항목 없음

---

## 2026-07-27 (107차) — 발표용 샘플계정(한가) 수입 거래에서 불필요한 구매처
필드 제거

사용자 요청: 홈 탭 스크린샷을 보여주며 "여기 보면 지금 내가 입력하는 부분은
배달매출로 입력이 되는데 니가 입력한 데이터는 배달의 민족이라는 굵은
글씨로 입력된거 보여? 이거 무슨 차이야?" → merchant(구매처) 필드가
있으면 그게 굵게, 없으면 category가 굵게 표시되는 차이라고 설명 →
"수정 범위(배달매출만 vs 수입 전체)"를 물었더니 "수입에 구입처가 어디있어?"
→ 수입 전체에서 merchant를 비우기로 결정

### 확인한 현재 상태
- 원격 D1 한가 계정(`8daf2fba-...`)의 수입 거래 314건(홀매출 113 +
  배달매출 160 + 포장매출 41) 전부 merchant가 채워져 있었음(홀매출/
  포장매출은 전부 "매장"이라는 의미 없는 값, 배달매출은 배달의민족/
  쿠팡이츠/요기요) — 사용자가 직접 입력한 항목은 merchant를 안 채워
  분류명이 굵게 나오는 것과 스타일이 달라 보임(`TransactionList.tsx:240`
  `{tx.merchant || tx.category}`)

### 계획
- 원격 D1에서 `UPDATE transactions SET merchant = '' WHERE user_id =
  '8daf2fba-...' AND type = 'income' AND merchant != ''` 실행(지출
  거래는 원래 구매처 개념이 자연스러우므로 건드리지 않음)
- 코드 변경 없음, 데이터만 가공

### 완료
- [x] 원격 D1에서 UPDATE 실행 — 수입 거래 316건 merchant를 빈 문자열로
  변경(317건 중 이미 빈 값이던 배달매출 1건 제외하면 316건 일치)
- [x] 검증 — 수입 317건 전부 merchant='' 확인, 지출 70건은 그대로
  merchant 유지(구매처 개념이 자연스러운 지출은 안 건드림)
- 이제 홈 탭에서 모든 수입 항목이 사용자가 직접 입력한 스타일과
  동일하게 분류명(홀매출/배달매출/포장매출)이 굵게 표시되고, 배달매출은
  결제방법 칩(배민/요기요/쿠팡이츠/계좌이체)으로만 주문처가 구분됨

---

## 2026-07-27 (106차) — 발표용 샘플계정(한가) 배달앱 매출을 카드정산기로
시연 가능하게 데이터 재가공

사용자 요청: "더미 데이터가 기능을 인지하지 못하고 이미 다 결산이 되어있는데
배달어플 주문한 금액은 카드정산기 기능을 활용할수 있게 결제 방법 칩 다시
선택하고 데이터 다시 가공해서 작업해줘 모든 기능 새로 시연 하면서 설명할수
있게"

### 확인한 현재 상태 (원격 D1, 한가 계정 user_id 8daf2fba-...)
- 사용자가 이미 직접 결제방법 "배민"/"요기요"/"쿠팡이츠"(수입)를 추가해두고
  (03:40경), 카드정산기 목표 결제방법도 "계좌이체"로 설정돼 있음(01:07경,
  99차 더미 데이터 생성 시점 설정으로 추정)
- `card_settlement_source_payment_methods`는 현재 비어있음(소스 미선택
  상태) — 발표 때 이 칩 선택 자체를 직접 시연하실 것으로 보여 미리
  선택해두지 않음
- 배달매출 거래 160건 전부 `payment_method='계좌이체'`(이미 정산 완료
  상태)라 지금 카드정산기에서 소스로 배민/요기요/쿠팡이츠를 선택해도
  뜨는 항목이 없음(전부 이미 계좌이체로 기록되어 있어 소스 필터에 안 걸림)
- 예외적으로 사용자가 이미 실제 화면에서 라이브 테스트해본 흔적 발견:
  - 2026-07-03 배달의민족/쿠팡이츠 2건은 `pending_source_payment_method`가
    정상적으로 "배민"/"쿠팡이츠"로 남아있음(정확한 확인 흐름으로 테스트됨)
  - 2026-07-16(3건)/07-17(2건)/07-19(1건) 6건은 `pending_source_payment_method`가
    "계좌이체"로 잘못 남아있음(정정 전 버전 — "계좌이체"를 소스로 잘못
    선택해 이미 정산된 항목을 다시 체크했던, 104차에서 고친 중복 버그의
    실제 재현 사례. 다행히 104차 수정 이후 테스트라 메모 중복은 없었음)

### 계획 (코드 변경 없음 — 원격 D1 데이터만 가공)
- Group A(잘못된 pending_source 정정, 6건: 07-16×3/07-17×2/07-19×1) —
  `pending_source_payment_method`를 "계좌이체"에서 실제 주문처에 맞는
  플랫폼("배민"/"요기요"/"쿠팡이츠")으로 정정. 발표 때 "이미 확인된 항목
  체크 해제(되돌리기)" 시연용으로 그대로 활용
- Group B(최근 정산완료 5건을 다시 미확인 상태로 되돌림, 07-25×2/07-26×3) —
  `payment_method`를 계좌이체 → 해당 플랫폼으로, 메모의 " 입금완료" 제거,
  `pending_source_payment_method` NULL로. 발표 때 "실제 체크해서 확인하는"
  시연용
- Group C(오늘 날짜 07-27 배달매출 신규 3건 추가) — 아직 이 날짜엔
  배달매출 거래가 하나도 없어서, 배민/요기요/쿠팡이츠 각 1건씩 미확인
  상태로 신규 추가(가장 생생한 "오늘자 미정산 매출" 시연용)
- 예상 변경: 원격 D1 `transactions` 테이블 UPDATE 11건 + INSERT 3건만
  (코드/스키마 변경 없음)

### 완료
- [x] Group A — 07-16×3/07-17×2/07-19×1 총 6건, `pending_source_payment_method`를
  "계좌이체"에서 실제 주문처(배민/요기요/쿠팡이츠)로 정정
- [x] Group B — 07-25×2/07-26×3 총 5건, `payment_method`를 계좌이체 →
  주문처로, 메모의 " 입금완료" 제거, `pending_source_payment_method` NULL로
  되돌려 미확인 상태로 전환
- [x] Group C — 2026-07-27(오늘, 발표일) 배달매출 신규 3건 추가(배민
  268,400원 / 쿠팡이츠 187,200원 / 요기요 112,900원), 전부 미확인 상태
- [x] 최종 검증 — 확인 대기(pending, payment_method가 배민/요기요/쿠팡이츠인
  것) 8건, 이미 확인됨(pending_source_payment_method 있음, 되돌리기
  데모용) 8건으로 균형있게 구성된 것 확인
- 발표 시연 순서 제안: 카드정산기 탭 → "카드매출 결제방법"에서 배민/요기요/
  쿠팡이츠 칩 3개 직접 선택(칩 선택 자체가 하나의 기능 시연) → 목표
  결제방법은 이미 "계좌이체"로 설정되어 있음 확인 → 미확인 8건 중 하나
  체크해서 "입금완료" 초록 강조 + 확인 흐름 보여주기 → 이미 확인된 8건
  중 하나 체크 해제해서 되돌리기 흐름도 보여주기 → 홈 탭에서 방금 확인한
  거래의 결제방법/메모가 실제로 바뀐 것 확인
- 코드/스키마 변경 없음, 원격 D1 데이터만 가공(로컬 D1은 건드리지 않음)

---

## 2026-07-27 (105차) — 홈 탭 데스크톱 레이아웃에서 입력 폼이 sticky로 고정돼
스크롤을 끝까지 내려야 내려가던 문제 수정

사용자 요청: "홈 부분에 입력하는 섹션이 있고 내역이 나오는 섹션이 있자나
스크롤을 끝까지 다 내려야 입력하는 부분이 아래로 내려가 이부분 문제인거
같은데?"

### 원인
- `src/App.tsx:452-453` — 홈 탭 레이아웃이 `lg:` 브레이크포인트(1024px)
  이상에서 `lg:grid lg:grid-cols-[420px_1fr]` 2컬럼 그리드가 되고, 왼쪽
  컬럼(SummaryCard + 버튼 + 예산배너 + TransactionForm 입력 폼)에
  `lg:sticky lg:top-20`이 걸려 있음
- 오른쪽 컬럼(TransactionList, 거래 목록)이 왼쪽보다 길면, 왼쪽 폼이
  화면 상단에 붙박힌 채 오른쪽 목록만 스크롤되다가, 그리드 컨테이너
  바닥(=거래 목록을 다 스크롤한 지점)에 도달해야 sticky가 풀리며 폼이
  아래로 이동 — 전형적인 sticky-sidebar 동작이 사용자가 신고한 증상과
  정확히 일치(1024px 이상 데스크탑/큰 태블릿 화면에서만 발생, 모바일은
  원래 영향 없음)
- 사용자에게 확인 결과 sticky를 완전히 제거하는 쪽으로 결정(입력 폼도
  모바일처럼 페이지와 함께 자연스럽게 스크롤되길 원함)

### 계획
- `src/App.tsx:453`의 `lg:sticky lg:top-20` 제거
- 예상 변경 파일: `src/App.tsx`

### 완료
- [x] `src/App.tsx:453` — `lg:sticky lg:top-20` 제거(`space-y-4`만 남김).
  데스크톱에서도 왼쪽 컬럼(요약/입력 폼)이 모바일처럼 페이지와 함께
  자연스럽게 스크롤됨(오른쪽 거래 목록과 독립적으로 고정되지 않음)
- [x] tsc 빌드 + oxlint 통과

---

## 2026-07-27 (104차) — 확인/체크 시 메모에 완료 문구가 중복 추가되던 문제 수정

사용자 요청: "잘되었네 근데 이전에 기록한거 입금 완료가 겹쳐서 나오는데 이건
해결 못하나?" → 확인 질문에 사용자 답변: "기존에 완료 시켜놓았던 내역에
입금완료가 찍혀있자나 거기에 초록색으로 입력된 정보가 나오는데 체크는 새로
받으니까 2번 나오는거지?"

### 원인
- 이전(단방향) 카드정산기는 체크 즉시 목록에서 거래를 제거했었는데, 그
  당시 이미 "입금완료"가 붙은 거래들 중 일부는 `pending_source_payment_method`
  컬럼이 이번 세션에 새로 추가되며 전부 NULL로 시작해 "미확인" 취급됨 —
  결제방법이 현재 소스 목록과 우연히 겹치면 체크박스가 다시 "미확인(빈
  체크박스)" 상태로 목록에 나타남
- `handleToggleSettlement`(확인 브랜치)와 `DeliveryView`의
  `handleToggleDelivery`(체크 브랜치) 둘 다 메모에 이미 완료 문구가
  있는지 확인하지 않고 무조건 append — 이미 "입금완료"/"배송완료"가
  붙어있는 상태에서 체크하면 문구가 중복됨(운영 DB 조회로는 아직 실제
  중복 저장된 사례는 없었으나, 재현 가능한 버그로 확인)

### 계획
- `CardSettlementView.tsx`의 확인 브랜치, `DeliveryView.tsx`의 체크
  브랜치 둘 다 append 전에 memo가 이미 완료 문구로 끝나는지 확인해
  중복이면 그대로 두도록 수정(idempotent)
- 예상 변경 파일: `src/components/CardSettlementView.tsx`,
  `src/components/DeliveryView.tsx`

### 완료
- [x] `CardSettlementView.tsx` 확인 브랜치, `DeliveryView.tsx` 체크
  브랜치 둘 다 append 전에 `memo === 완료문구 || memo.endsWith(' ' + 완료문구)`로
  이미 붙어있는지 확인해 중복이면 그대로 두도록 수정
- [x] tsc 빌드 + oxlint 통과
- [x] `wrangler pages dev` 로컬 서버로 재현 시나리오 검증 — 메모가 이미
  "...정산 입금완료"로 끝나는 거래를 만들고 확인(체크) API 호출 →
  메모에 "입금완료"가 정확히 1번만 남아있는 것 확인(수정 전이었다면
  "...정산 입금완료 입금완료"로 중복됐을 상황)
- [x] 운영 D1 조회로 현재 실제 중복 저장된 거래는 없음을 확인(재현
  가능한 잠재 버그였고, 아직 실제 데이터 오염은 없었음 — 별도 데이터
  정리 불필요)
...
배송 완료 라고 메모 에 추가 해줘야 할거같은데 가능할까?"

### 계획
- `src/components/DeliveryView.tsx`의 `handleToggleDelivery`를 수정해
  체크(배송완료 처리) 시 `updateTransaction` 호출에 memo도 함께 갱신 —
  카드 정산기(`CardSettlementView.tsx`)의
  `tx.memo ? \`${tx.memo} 입금완료\` : '입금완료'` 패턴을 참고해
  `배송완료` 텍스트를 append
- 배송 탭은 체크 해제(토글)도 가능한 구조라 카드 정산기와 달리, 체크
  해제 시 자동으로 추가됐던 "배송완료" 텍스트를 다시 제거하는 로직도
  추가(사용자 확인 완료 — 체크 해제 시 자동 제거하는 방식 선택)
- 예상 변경 파일: `src/components/DeliveryView.tsx`
...
## 2026-07-27 (99차) — 발표용 샘플계정(한가) 더미 데이터 생성

사용자 요청: "한가 라는 계정이 내가 발표할 때 쓸 샘플계정이거든 이거 기술 시연용
더미 데이터 만들어서 보여줘야 할거 같아 니가 치킨집 사장님이라고 생각하고
내역 작성해줘"

### 확인한 현재 상태
- 원격 D1의 `users` 테이블에서 nickname = '한가' 계정 확인
  (user_id: `8daf2fba-4e95-4ecd-b416-81e93a6bf596`, email: aaa@aaa)
- 해당 계정은 거래/카드 0건인 완전히 빈 계정 — 발표용 더미 데이터를
  새로 채워도 기존 데이터와 섞일 위험 없음

### 계획 (코드 변경 없음 — 원격 D1 데이터만 삽입)
- 페르소나: 치킨집("치킨집 사장님") 개인사업자 가계부
- `cards` — 사업자카드 1개 등록
- `categories` — 업종 특화 커스텀 분류 추가
  - 수입: 홀매출, 배달매출, 포장매출
  - 지출: 재료비, 인건비, 임대료, 공과금, 배달수수료, 소모품, 광고비
- `merchants` — 도매생닭, 배달의민족, 쿠팡이츠, 요기요, 이마트, 한전, 도시가스 등
- `recurring_transactions` — 임대료(매달 1일) 고정지출 1건 등록해 고정 탭 기능도 시연
- `transactions` — 2026-05-01 ~ 2026-07-27 약 3개월치 일별 매출(홀/배달/포장,
  요일별 매출 편차 + 월요일 정기휴무 반영)과 지출(재료비/인건비/공과금/배달수수료/
  소모품/광고비) 현실감 있게 생성해 원격 D1에 직접 INSERT
- 변경 파일: 코드 변경 없음(WORKLOG.md만). 데이터는 `wrangler d1 execute
  budget-db --remote`로 직접 삽입, 삽입용 스크립트는 스크래치패드에만 보관

### 완료
- [x] 원격 D1에 직접 INSERT 완료 (계정: 한가, user_id `8daf2fba-4e95-4ecd-b416-81e93a6bf596`)
  - `cards` 1건 — 사업자카드(주황, 결제일 25일/마감일 10일)
  - `categories` 10건 — 수입(홀매출/배달매출/포장매출), 지출(재료비/인건비/
    임대료/공과금/배달수수료/소모품/광고비)
  - `merchants` 12건 — 도매생닭유통/이마트/코스트코/배달의민족/쿠팡이츠/
    요기요/알바생/건물주/한전/도시가스/수도사업소/청호나이스
  - `recurring_transactions` 2건 — 가게 임대료(매달 1일, 200만원),
    정수기 렌탈료(매달 5일, 3.3만원) → 로그인 후 첫 거래 조회 시
    `generateDueRecurringTransactions`가 5~7월분을 자동 생성해 고정 탭
    기능을 시연 가능
  - `transactions` 378건 (2026-05-01~2026-07-27, 매주 월요일 정기휴무 반영,
    금/토/일 매출 증가, 생닭 입고(화/금)·인건비(매달 10·25일)·전기/가스
    요금(매달 8·12일)·광고비(매달 3일)·배달수수료 정산(매달 28일) 등 실제
    치킨집 운영 패턴 반영)
  - 3개월 합계: 매출 55,970,199원 / 지출 48,780,308원 (순이익 약 719만원)
- [x] 생성용 스크립트는 스크래치패드에만 남기고 저장소에는 커밋하지 않음
      (재실행 시 매번 새 데이터가 중복 삽입되므로 1회성 스크립트로 취급)
- 코드 변경 없음(WORKLOG.md만 커밋)

사용자 요청: "메모에서 메모를 기록하고 1개 입력하고 저장하면 다음걸
작성하려고 했는데 위치를 최 상단으로 올라가버려"

### 원인
- `NotesView.tsx`가 `if (loading) { return <p>...불러오는 중...</p> }`로
  로딩 중엔 컴포넌트 전체를 한 줄짜리 로딩 문구로 통째로 교체함
- 메모 저장(`handleSave`)이 성공하면 매번 `load()`를 다시 호출해
  `loading`을 true로 만드는데, 이때 31일치 목록 전체가 순간적으로
  한 줄로 쪼그라들었다가 다시 채워짐 — 스크롤 위치(px)는 그대로지만
  콘텐츠 높이가 확 줄어드니 화면상 최상단으로 튀어 보이는 것.
  초기 진입 시 로딩 스피너를 보여주려던 의도였는데, 이후 재조회
  (저장 후 새로고침)에도 똑같이 적용된 게 문제
- 삭제(`handleDelete`)는 이미 `load()`를 다시 안 부르고 로컬 배열만
  낙관적으로 갱신해서 이 문제가 없음 — 저장 경로만 문제

### 계획
- `src/components/NotesView.tsx` — 전체 화면을 로딩 문구로 덮는 조건을
  `loading` 단독이 아니라 `loading && notes.length === 0`으로 변경.
  이러면 최초 진입(데이터 없음)엔 기존처럼 스피너가 보이고, 이미
  목록이 있는 상태에서의 재조회(저장 후, 월 이동 등)는 기존 목록을
  그대로 유지한 채 배경에서 갱신되어(스테일-와일-리밸리데이트) 스크롤
  위치가 보존됨
- `wrangler pages dev` + Playwright로 메모 저장 후 스크롤 위치가
  유지되는지, 최초 진입 시 로딩 스피너는 정상적으로 보이는지 검증

### 완료
- [x] `src/components/NotesView.tsx` — `notes.length === 0` 대신
  `hasLoadedOnceRef`(최초 로드 완료 여부를 추적하는 ref)로 기준 변경.
  최초 진입 시에만 로딩 문구로 전체 화면을 덮고, 그 이후 모든 재조회는
  그 달에 메모가 0건이었더라도 화면을 그대로 유지하도록 함(단순
  `notes.length === 0` 체크로는 "그 달의 첫 메모를 저장하는 순간"에
  여전히 화면이 튀는 엣지케이스가 남아있어 ref 기반으로 더 견고하게 수정)
- [x] `npx tsc -b --noEmit`, `npm run lint` 모두 통과
- [x] `wrangler pages dev` + Playwright로 검증: 스크롤을 20일 위치까지
  내린 뒤 그 달의 첫 메모를 저장해도 스크롤 위치가 그대로 유지됨(이전
  버그 재현 시 896→0으로 튀었으나 수정 후 896→896 유지), 이어서 같은
  위치에서 두 번째 메모를 저장해도 마찬가지로 유지됨(952→952, 실제
  사용자가 겪은 "다음 메모 작성 시 튐" 시나리오), 메모 탭 최초 진입
  시엔 기존처럼 로딩 스피너가 정상적으로 보였다가 목록이 렌더링됨을 확인
- 미완료 항목 없음(스키마/마이그레이션 변경 없음)

## 2026-07-23 (97차) — 고정 탭 목록에서 계좌이체/현금 결제방법 표시 누락 수정

사용자 요청(스크린샷 첨부): "NH카드는 표시가 나오는데 계좌는 와 안나오노?
이거 한번 확인해봐라 여기 고정탭이야"

### 원인
- `RecurringManager.tsx`의 고정항목 목록 렌더링에서 결제방법 뱃지가
  `{card && (...NH 같은 카드 이름...)}` 조건으로만 표시되고 있었음 —
  즉 결제방법이 등록된 카드일 때만 뱃지가 나오고, 계좌이체/현금/
  커스텀 결제방법(96차에서 막 추가한 기능)일 때는 뱃지 자체가 아예
  없어서 리스트에 결제방법 정보가 통째로 안 보였음
- `TransactionList.tsx`는 이미 등록된 카드면 카드색 뱃지, 아니면
  `item.payment_method || '현금'` 텍스트 뱃지를 항상 보여주는데
  RecurringManager.tsx만 이 fallback이 빠져 있었음(96차에서 관리
  기능은 고쳤지만 목록 표시 쪽은 놓쳤던 부분)

### 계획
- `src/components/RecurringManager.tsx` — 목록 렌더링에서 `card &&`
  단독 조건을 `card ? (카드 뱃지) : (item.payment_method || '현금'
  텍스트 뱃지)`로 교체(TransactionList.tsx와 동일한 fallback 패턴)
- `wrangler pages dev` + Playwright로 카드/계좌이체/현금/커스텀
  결제방법 각각으로 등록한 고정항목이 목록에서 전부 뱃지로 보이는지 검증

### 완료
- [x] `src/components/RecurringManager.tsx` — 목록의 결제방법 뱃지를
  `card ? (카드색 뱃지) : (item.payment_method || '현금' 회색 텍스트
  뱃지)`로 교체해 카드가 아닌 모든 결제방법도 항상 표시되도록 수정
- [x] `npx tsc -b --noEmit`, `npm run lint` 모두 통과
- [x] `wrangler pages dev` + Playwright로 검증: 계좌이체로 등록한
  고정항목엔 "계좌이체" 회색 뱃지, 현금(기본값)으로 등록한 항목엔
  "현금" 뱃지가 각각 목록에 정상 표시됨을 확인(이전엔 카드가 아니면
  뱃지 자체가 없었음)
- 미완료 항목 없음(스키마/마이그레이션 변경 없음)
...
### 완료
- [x] `src/components/RecurringManager.tsx` — 분류/구매처/결제방법 모두
  `ReorderableChipList` + 관리모드(Settings2 토글) + "+ 직접입력" 추가
  패턴으로 교체(TransactionForm.tsx와 동일), 구매처는 지출일 때만
  노출(수입엔 판매처 개념 없음, 기존 원칙과 통일)
- [x] 결제방법 하드코딩 버그 수정 — `getPaymentMethods(form.type)` 기반
  동적 목록으로 교체(계좌이체·커스텀 결제방법 선택 가능해짐)
- [x] 저장 시 커스텀 결제방법이 '현금'으로 되돌아가던 버그 수정
  (`selectedCard ? selectedCard.id : form.payment_method`)
- [x] 수정 진입 시 `item.payment_method`를 무시하던 버그 수정
  (`item.card_id || item.payment_method || '현금'`)
- [x] `handleTypeChange`에 분류/결제방법 목록 재동기화 + 관리모드 리셋 추가
  (TransactionForm과 동일하게 지출↔수입 전환 시 목록이 올바르게 갱신)
- [x] `npx tsc -b --noEmit`, `npm run lint` 모두 통과
- [x] `wrangler pages dev` + Playwright로 검증: 결제 방법에 "계좌이체"
  기본 옵션이 정상 노출됨(이전엔 없었음), "+ 직접입력"으로 커스텀
  결제방법("제로페이") 추가 시 자동 선택됨, 저장 후 항목을 다시
  수정 화면으로 열었을 때 "제로페이"가 그대로 유지됨(현금으로
  되돌아가던 버그 해결 확인), 분류 관리모드(삭제 모드) 진입도
  정상 동작
  - 검증 중 테스트 스크립트가 칩의 wrapper `<div>`와 실제 칩
    `<div>`를 구분 못해 선택 상태를 잘못 판정하는 문제가 있었음 —
    `.find()` 대신 `.filter().some()`으로 모든 매칭 요소를 확인하도록
    수정해 재검증 완료(앱 코드 문제 아님, 테스트 스크립트만 수정)
- 미완료 항목 없음(스키마/마이그레이션 변경 없음)
...
"WORKLOG.md를 참고하라고 링크했는데, 정작 그 파일 자체가 리포지토리에
공개되는지는 확인이 필요해요. ... WORKLOG.md를 리드미에서 한 번 더
강조하거나(예: '76개 세션, N개 버그 수정 기록' 같은 숫자로 요약) 링크를
걸어주면 신뢰도가 한 번 더 올라갈 것 같아요."

### 확인
- `git ls-files WORKLOG.md` / `git check-ignore`로 WORKLOG.md가 정상
  추적되고 있으며 gitignore 대상이 아님을 확인
- `gh repo view`로 저장소가 PUBLIC임을 확인 — WORKLOG.md는 이미
  `https://github.com/heo-hyuk/budge/blob/main/WORKLOG.md`에서 열람 가능
- 실제 통계 집계: 세션 헤더(`## YYYY-MM-DD`) 102개(그중 93개는 "N차" 번호,
  최신 93차), 기간 2026-07-14~07-23, 전체 커밋 197개 중 `fix:` 36건

### 완료
- [x] README.md 상단(배포 URL 아래)에 WORKLOG.md 링크 + 세션/커밋/버그
  수정 건수 요약 블록 추가
- [x] "개발 / 검증 프로세스" 절 말미에도 "발췌이며 전체는 WORKLOG.md에서
  확인 가능" 링크 문구 추가
- 미완료 항목 없음

## 2026-07-23 (93차) — README에 테스트/검증 과정에서 잡은 버그 사례 반영

사용자 요청: "워크로그에 보면 수정 많이 했자나 이것도 테스트로 반영해서
오류 잡은거 적어줘 리드미에" — WORKLOG.md에 쌓인 `wrangler pages dev` +
Playwright 검증 기록 중 실제로 발견·수정된 버그 사례를 README에 정리해
개발/검증 프로세스를 문서화해달라는 요청.

### 완료
- [x] `README.md`에 "## 개발 / 검증 프로세스" 섹션 신설 — 배포 전 로컬
  `wrangler pages dev` + Playwright 검증 방식 설명과 함께, WORKLOG.md에서
  실제로 검증 중 발견해 고친 버그 8건을 대표 사례로 정리(카드 청구기간
  시작일 클램핑 오류, 계좌이체 혜택매칭 오인, 분류 삭제 ON CONFLICT 버그,
  이중 confirm() 삭제 버그, 일일정산 전일잔액 리셋, PWA standalone
  confirm() 무반응, 다크모드 드롭다운 글자 안 보임, 빠른입력 템플릿 메모/
  결제방법 누락)
- 미완료 항목 없음
...
항목확인에 좋을거 같아 메모도 같이보여줘"

### 계획
- `src/components/DeliveryView.tsx` — 거래 목록의 각 항목에
  TransactionList.tsx와 동일한 패턴으로 메모(`tx.memo`)가 있을 때만
  표시
...
- [x] **긴급**: 90차 코드(`functions/lib/settlement.ts`,
  `functions/api/export/index.ts`)가 이미 새 테이블명
  (`card_settlement_source_payment_methods`)을 참조한 채 배포됐는데,
  89차에서 보류했던 원격 rename 마이그레이션(027)이 아직 운영 DB에
  적용 안 된 상태였음을 뒤늦게 인지 — 이 상태로는 정산(일/주/월/연)·
  내보내기 등 핵심 API 전체가 "no such table" SQL 에러로 깨지는
  상황이라 사용자 확인을 기다리지 않고 즉시
  `wrangler d1 execute budget-db --remote --file=./migrations/027_card_settlement_payment_method.sql`
  실행(4개 쿼리 정상 처리, 기존 88차 데이터 rename만 되고 유실 없음).
  운영 사이트(`https://budget-3wb.pages.dev`)의 `/api/settlement/monthly`가
  인증 미들웨어까지 정상 도달(401)하는 것으로 1차 확인, 실제 계정
  생성까지는 하지 않음(운영 데이터에 테스트 계정을 만들지 않기 위함)
- 미완료 항목 없음
...
- [x] 원격(production) D1 마이그레이션(027) 적용 완료 — 90차 작업
  도중 이 마이그레이션이 늦어져 운영 API가 깨질 위험을 발견해
  긴급하게 먼저 적용함(자세한 경위는 90차 항목 참고)
...
- [x] 사용자 확인 후 원격(production) D1에
  `wrangler d1 execute budget-db --remote --file=./migrations/025_add_delivery.sql`
  실행 완료(3개 쿼리 정상 처리, 기존 데이터 변경 없이 컬럼/테이블만 추가됨)
- 미완료 항목 없음

## 2026-07-22 (86차) — 메모 첨부 사진 보기에 닫기 버튼/더블클릭 닫기 추가

사용자 요청: "메모탭에 사진 첨부 기능이 있자나 근데 사진을 열고 닫으려면
닫기버튼이 필요한데 없어 그리고 더블 클릭하면 꺼지는것도"

### 원인 분석
- `NotesView.tsx`의 `renderNoteItem`에서 첨부 이미지를
  `<a href=... target="_blank">` + `<img>`로 렌더링하고 있어, 탭하면
  브라우저의 "새 탭에서 이미지 열기" 기본 동작에 의존하고 있었음
- 이 앱은 홈 화면에 설치해 쓰는 PWA(75차 이후 계속 다룸)라 standalone
  모드에서는 새 탭이 열려도 주소창/뒤로가기 같은 브라우저 UI가 없어
  사용자가 그 화면을 벗어날 방법이 없었음 — "닫기 버튼이 없다"는
  증상의 정확한 원인

### 계획
- 이미지 첨부 표시를 앵커(`<a target="_blank">`)에서 앱 내부 전체화면
  뷰어(모달)로 교체: 썸네일을 탭하면 오버레이로 원본 이미지를 크게
  보여주고, 우상단에 명시적 닫기(X) 버튼 추가
- 배경(오버레이) 클릭 시에도 닫히도록(ConfirmDialog.tsx와 동일한
  패턴: role="presentation" + onClick으로 배경 클릭 닫힘, 이미지
  자체 클릭은 stopPropagation으로 무시)
- 이미지를 더블클릭(더블탭)하면 바로 닫히도록 `onDoubleClick` 핸들러
  추가
- `wrangler pages dev` + Playwright(터치 기기 에뮬레이션 포함)로 썸네일
...
### 설계
- 수입계산기는 기존 그대로(기본 전부 미선택, 칩을 눌러 포함시키는
  "포함 방식") 유지 — 이번 요청은 지출계산기에만 해당
- 지출계산기는 반대로 "제외 방식"으로 동작: 기본 상태에서 모든 지출
  분류가 합계에 포함되어 있고, 칩을 탭하면 그 분류만 합계에서 제외됨
  (다시 탭하면 재포함)
- 백엔드(calc_selections 테이블/API)는 변경 없음 — 어차피 "분류명
  하나를 저장/삭제"하는 것 뿐이라 의미만 다르게 해석하면 됨:
  수입은 저장된 행 = "포함된 분류", 지출은 저장된 행 = "제외된 분류"로
  해석. `src/lib/calcSelections.ts`는 그대로 두고
  `CategoryCalculator.tsx`에서 타입별로 포함 여부 계산 방식만 다르게 함
- 안내 문구도 지출계산기만 "기본적으로 전체 포함, 제외하고 싶은 분류만
  탭해서 꺼주세요" 식으로 변경

### 계획
- `src/components/CategoryCalculator.tsx` — THEME에 `mode:
  'include' | 'expense'` 성격의 필드 추가해 지출은 "제외 방식"으로
  분기, 칩 강조 표시·합계·일별 표에 쓰이는 "포함된 분류 목록" 계산
  로직을 모드에 따라 뒤집기
- `wrangler pages dev` + Playwright로 지출계산기 첫 진입 시 전체 분류가
  합계에 포함되어 있는지, 칩 하나를 꺼도 다른 분류엔 영향 없는지,
  새로고침 후에도 제외 상태가 유지되는지, 수입계산기는 회귀 없이 기존
  동작(기본 미선택) 그대로인지 검증

### 완료
- [x] `src/components/CategoryCalculator.tsx` — THEME에 `mode:
  'include' | 'exclude'` 필드 추가(수입=include, 지출=exclude),
  `isIncluded(category)` 헬퍼로 포함 여부 판정을 모드에 따라 뒤집도록
  일반화. calc_selections엔 여전히 "탭해서 생긴 행"만 저장되고
  의미만 다르게 해석(수입=포함된 분류, 지출=제외된 분류)
- [x] 지출계산기 안내 문구를 "기본적으로 모든 지출 분류가 포함, 제외할
  분류만 탭해서 꺼주세요" 식으로 변경, 전체 제외 시 안내 메시지도
  "모든 지출 분류가 제외되어 있습니다"로 별도 처리
- [x] `npx tsc -b --noEmit`, `npm run lint` 모두 통과
- [x] `wrangler pages dev` + Playwright로 실제 서버 검증: 지출계산기
  첫 진입 시 등록된 지출 분류 전체(9개)가 자동으로 선택된 상태로
  합계에 포함됨을 확인, 식비 칩 하나만 꺼도 교통 등 다른 분류는
  그대로 유지됨(합계가 13,000원→3,000원으로 정확히 갱신), 새로고침
  후에도 제외 상태가 유지됨(D1 동기화), 수입계산기는 회귀 없이 기존
  동작(기본 전부 미선택) 그대로 유지됨을 확인
  - 검증 중 테스트 스크립트가 분류 칩을 `<button>` 셀렉터로 찾아
    실패한 최초 오류 발견 — 이 앱의 분류 칩은
    `ReorderableChipList`가 렌더링하는 `<div className="inline-flex...">`
    이지 `<button>`이 아님. 셀렉터를 `div.inline-flex` 기준으로
    수정해 재검증 완료(앱 코드 문제 아님, 테스트 스크립트만 수정)
- 미완료 항목 없음
...
너무 힘드네 내용이 많아지니까". 거래 목록·검색 결과 등 내용이 길어진
화면에서 스크롤이 힘들다는 피드백 — 탭과 무관하게 앱 전체에서 쓸 수
있는 공통 기능으로 판단.

### 설계
- 특정 탭 전용이 아니라 어느 화면에서든 쓰이도록 `App.tsx`에 전역으로
  하나만 배치(각 탭/컴포넌트마다 따로 넣지 않음)
- 우측 하단에 떠 있는 원형 버튼 2개(위/아래 화살표) — `window.scrollY`와
  문서 전체 높이를 기준으로, 위로 어느 정도 내려갔을 때만 "맨 위로"를,
  아직 맨 아래 근처가 아닐 때만 "맨 아래로"를 보여줌(둘 다 필요 없으면
  버튼 자체를 안 그림 — 짧은 화면에서 불필요한 UI 안 뜨게)
- `window.scrollTo({ behavior: 'smooth' })`로 부드럽게 이동

### 계획
- `src/components/ScrollButtons.tsx` 신규 — 스크롤 위치 추적 + 위/아래
  버튼 렌더링
- `src/App.tsx` — 메인 콘텐츠 영역 아래에 `<ScrollButtons />` 배치
- `wrangler pages dev` + Playwright로 짧은 화면(버튼 안 보임)과 긴
  화면(스크롤에 따라 버튼 표시/숨김, 클릭 시 실제 이동) 검증

### 완료
- [x] `src/components/ScrollButtons.tsx` 신규 — `scroll`/`resize` 이벤트로
  `window.scrollY`와 문서 높이를 추적해 "맨 위로"(300px 이상 내려갔을
  때)/"맨 아래로"(맨 아래 300px 이내가 아닐 때) 버튼을 조건부 표시
- [x] `src/App.tsx` — 메인 콘텐츠 영역 뒤에 `<ScrollButtons />` 전역 배치
- [x] `wrangler pages dev` + Playwright로 검증: 맨 위에 있을 때 "맨 위로"
  버튼 숨김·"맨 아래로"만 표시, 중간 스크롤 시 둘 다 표시, 각 버튼
  클릭 시 실제로 맨 위/맨 아래까지 정확히 스크롤됨(거래 15건 등록해
  긴 목록으로 재현). `tsc -b`/`oxlint`/`npm run build` 모두 통과.
  스키마 변경 없음

## 2026-07-21 (81차) — 검색 화면 결제 방법 필터가 결제 방법 관리 기능과 연결 안 된 문제 수정

사용자 질문: "지금 칩 편집기능이 여기저기 많이 있잖아 근데 여기서 위치이동하고
세팅하는게 공통적으로 유지되지 않는데 이건 데이터베이스에 연결안되어 있어?"
— 칩 재정렬/추가가 실제로 서버에 저장/동기화되는지에 대한 의문.

### 조사
- Playwright로 직접 검증: 분류/구매처 드래그 재정렬 → 새로고침 → 새 기기
  (새 로그인 세션) → 검색 화면 분류/구매처 필터 → 거래 목록 인라인 수정
  화면까지 전부 정확히 반영되는 것 확인(DB 연결·동기화 자체는 정상)
- 딱 하나 발견: **검색 화면(`SearchView.tsx`)의 "결제 방법" 필터**만
  68차에서 만든 결제 방법 관리 시스템(`getPaymentMethods`)과 연결이
  안 되어 있고, "현금"/"계좌이체" + 등록된 카드만 하드코딩돼 있었음 —
  68차 이전부터 있던 코드가 그대로 남아있었던 것. 이 필터에서는 커스텀
  결제 방법이 아예 안 보이고, 현금/계좌이체 순서 변경도 반영 안 됨

### 수정
- `src/components/SearchView.tsx`:
  - `getPaymentMethods` 임포트, `categoryList()`와 동일한 패턴으로
    `paymentMethodList()` 추가(구분 필터에 따라 지출/수입/전체 결제
    방법 목록 반환)
  - 하드코딩된 "현금"/"계좌이체" 버튼 2개를 `paymentMethodList().map()`
    기반 동적 칩으로 교체(카드 칩은 기존처럼 별도로 이어붙임)
  - 필터링 로직의 `'cash'`/`'transfer'` 센티널 방식을 결제 방법 이름
    문자열 직접 비교로 일반화(등록된 카드는 기존처럼 `card_id`로 서버
    필터링, 그 외엔 `payment_method` 문자열 비교)
- `wrangler pages dev` + Playwright로 커스텀 결제 방법 추가/재정렬이
  검색 필터에 정확히 반영되는지, 기존 현금/계좌이체/카드 필터링도
  회귀 없는지 검증

### 완료
- [x] `src/components/SearchView.tsx` — `paymentMethodList()` 추가,
  하드코딩된 현금/계좌이체 버튼을 동적 칩으로 교체, 서버/클라이언트
  필터링 로직을 결제 방법 이름 문자열 비교로 일반화(등록된 카드는
  기존처럼 `card_id`로 서버 필터링 유지)
- [x] `wrangler pages dev` + Playwright로 검증: 지출 결제 방법에 커스텀
  "제로페이" 추가 → 검색 필터 칩에 즉시 노출 → 필터 선택 시 정확히
  해당 결제수단 거래만 조회됨. 기존 현금/계좌이체 필터도 각각 정확한
  거래만 걸러내는지 재확인(회귀 없음 — 첫 시도에서 테스트 스크립트가
  결제 방법 칩을 `.click()`으로 눌러 실제로는 선택이 안 된 채 저장되는
  바람에 거짓 회귀로 보였던 것을 포인터 기반 클릭으로 재확인해 바로잡음)
- [x] `tsc -b`/`oxlint`/`npm run build` 모두 통과. 스키마 변경 없음

## 2026-07-21 (80차) — 날짜 input이 카드 밖으로 삐져나오는 iOS 전용 문제 수정(79차 후속)

79차(헤더 오버플로) 배포 후 사용자 제보: 거래 입력 폼의 "날짜" 입력창이
카드 경계를 넘어감(사진 확인). "아이폰만 넘치던데" — 기능(탭하면 달력이
뜨고 저장도 잘 됨)은 정상, 순수하게 그 입력창 박스만 카드 밖으로 삐져나와
보인다고 확인.

### 확인 및 판단
- 실제 WebKit으로 `#date` input의 `getBoundingClientRect()`를 직접
  측정해보니 레이아웃 박스 자체는 카드 안에 정상적으로 들어가 있었음
  (오른쪽 여백 17px 남음) — 이 환경(리눅스 WebKitGTK)의 `<input
  type="date">` 렌더링은 iOS와 완전히 다른 네이티브 위젯(GTK 자체
  달력 위젯)을 쓰기 때문에, iOS Safari 고유의 렌더링 버그까지는 이
  환경에서 재현이 안 됨
- `:focus-visible` 아웃라인(탭에는 안 뜸)도 원인에서 제외
- 사용자가 반복적으로 "그 박스만" 넘친다고 명확히 확인해줬고, 기능은
  정상이라고 해서 순수 시각적 렌더링 문제로 판단 — `<input
  type="date">`류 네이티브 폼 컨트롤은 iOS Safari에서 OS가 자체적으로
  계산한 최소 폭(달력 아이콘·연월일 세그먼트를 위한)을 CSS `width`보다
  우선시해 카드 밖으로 삐져나오는 경우가 있는 것으로 잘 알려진 WebKit
  특성 — `-webkit-appearance: none`으로 네이티브 외형을 초기화해야
  CSS가 지정한 폭을 실제로 존중하게 됨

### 수정
- `src/index.css`에 전역 규칙 추가: `input[type="date"] { -webkit-appearance:
  none; appearance: none; }` — 개별 컴포넌트 className을 8곳 각각 고치는
  대신, 앱에 있는 모든 날짜 입력(거래 입력 폼, 거래 목록 인라인 수정,
  내보내기, 고정 지출, 검색 필터)에 한 번에 일괄 적용
- iOS 실기기 검증은 이 환경에서 불가능 — 사용자 확인 필요

### 완료
- [x] `src/index.css`에 `input[type="date"] { -webkit-appearance: none;
  appearance: none; }` 전역 규칙 추가
- [x] 실제 WebKit으로 재검증: 레이아웃 박스가 여전히 카드 안에 정상
  위치(회귀 없음), 값 설정/읽기(`fill`/`inputValue`)도 정상 동작 확인
- [x] `tsc -b`/`oxlint`/`npm run build` 통과
- [x] 이 CSS만으로 iOS의 실제 네이티브 위젯 렌더링까지 재현·확정 검증은
  이 환경에서 불가능 — 사용자가 실기기에서 확인 필요. 만약 이걸로도
  안 고쳐지면 네이티브 `<input type="date">` 자체를 커스텀 날짜
  선택 UI로 교체하는 더 큰 작업이 다음 단계

## 2026-07-21 (79차) — 헤더 가로 오버플로 실제 원인 발견·수정 (75~78차 후속, 근본 해결)

78차에서 사용자가 "크롬도 다른 브라우저도 다 그래 이건 아이폰만 그런듯"이라고
확인 — 사파리 전용 설정이 아니라 iOS(WebKit 엔진) 자체 문제로 확정. 코드
추측을 멈추고 이 환경에서 실제 WebKit 엔진을 직접 구동해 재현·특정한 뒤
근본 원인을 고쳤다.

### 실제 WebKit 구동 성공
- Playwright의 webkit 브라우저 바이너리는 다운로드 자체는 완료됐지만
  실행에 필요한 `libgstcodecparsers-1.0.so.0`(GStreamer 코덱 파서)이
  시스템에 없어 launch가 실패. sudo 권한이 없어 `apt-get install`은
  불가했지만, `apt-get download`(다운로드만, 설치 아님)는 root 없이도
  동작해서 .deb 패키지를 받아 사용자 홈 디렉터리에 직접 풀어(dpkg-deb -x)
  라이브러리 확보
- Playwright의 webkit 실행 파일 체인을 추적한 결과, 최종적으로
  `minibrowser-wpe/MiniBrowser`라는 쉘 스크립트가 `LD_LIBRARY_PATH`를
  **덮어쓰기**(추가가 아니라 대입)하고 있어서, 상위 스크립트(`pw_run.sh`)나
  Node의 `env` 옵션으로 아무리 라이브러리 경로를 넘겨도 씹히는 게
  원인이었음 — 이 wrapper 스크립트를 직접 패치(경로 추가)해서 해결.
  이 환경 안에서만 유효한 임시 조치이며 프로젝트 코드와는 무관
- 이렇게 확보한 실제 WebKit 엔진으로 아이폰 13프로와 동일한 뷰포트
  (390×844, DPR 3)를 재현해 배포된 사이트를 열어보니 **실제로
  `scrollWidth=427`인데 `innerWidth=390`** — 37px의 진짜 가로 오버플로가
  확인됨(Chromium에서는 이 수치가 재현 안 됐던 이유는 별도 확인 — 크롬은
  콘텐츠가 넘치면 `innerWidth` 자체를 넓혀 흡수하는 반면, iOS Safari/WebKit은
  `width=device-width`를 엄격히 지켜 `innerWidth`는 그대로 두고 대신 페이지
  전체를 줌아웃해서 넘치는 콘텐츠를 욱여넣음 — 이게 사용자가 본 "전체
  화면이 찌그러진" 증상의 정체였음. 안드로이드 다양한 기종·심지어 더
  좁은 폴드 커버 화면도 문제없었던 이유이기도 함 — 오버플로 자체는 항상
  있었지만 안드로이드/크롬 계열은 시각적으로 티가 안 나게 흡수한 것)

### 근본 원인
- 헤더(`App.tsx`)의 상단 바 — 햄버거+로고, 월 이동(◀ 라벨 ▶ 오늘),
  다크모드 토글, 닉네임 드롭다운 — 4개 그룹이 전부(또는 대부분)
  `shrink-0`라 줄어들 수 없었고, 390px처럼 좁은 화면에서 이 4개 그룹의
  실제 필요 폭 합이 뷰포트보다 커서 넘쳤음. 특히 닉네임 드롭다운
  wrapper(`relative shrink-0`)가 화면 오른쪽 끝에서 그대로 잘려나가고
  있었음(개발자 도구로 요소 하나하나 bounding box를 찍어 특정)
- 처음엔 닉네임 wrapper만 `shrink`로 바꿨는데도 오버플로가 그대로였음 —
  원인을 더 파보니 wrapper(div)는 flexbox 계산으로 실제 6px까지
  줄어들었지만, 그 안의 `<button>`은 `overflow` 제약이 없어서 부모가
  줄어든 폭과 무관하게 자기 콘텐츠(닉네임 텍스트+화살표) 원래 크기 그대로
  렌더링돼 부모 박스 밖으로 삐져나오고 있었음 — `w-full overflow-hidden`을
  버튼에 추가해야 부모가 계산한 폭만큼만 렌더링되고, 그 안의 텍스트도
  `truncate`가 실제로 적용됨

### 수정
- `src/App.tsx`:
  - 헤더 행 전체 gap을 `gap-2`→`gap-1`로 살짝 축소
  - 월 이동 라벨 `min-w-24`(96px)→`min-w-20`(80px), ◀/▶/오늘 버튼
    패딩 `px-2.5`→`px-2`, 오늘 버튼에 `shrink-0 whitespace-nowrap` 추가
    (줄바꿈으로 두 줄이 되던 것 방지)
  - 닉네임 wrapper: `shrink-0` → `min-w-0 shrink`(줄어들 수 있게),
    버튼에 `w-full overflow-hidden` 추가, 닉네임 텍스트를 `<span
    className="truncate">`로 감싸고 화살표는 `shrink-0`으로 고정 —
    공간이 부족하면 닉네임 텍스트가 우선적으로 줄어들다 못해 사라지는
    한이 있어도 페이지 자체는 절대 넘치지 않게 함(극단적으로 긴
    닉네임 + 12월 라벨 + "오늘" 버튼이 동시에 뜨는 최악의 조합까지
    직접 재현해서 확인)
- 스키마/설정 변경 없음 — 순수 CSS(Tailwind 클래스) 수정

### 검증
- 실제 WebKit 엔진, 아이폰 13프로 뷰포트(390×844)로: 기본 상태(홈,
  실제 고객 닉네임 "상희")부터 9개 탭 전부 `scrollWidth === innerWidth`
  (오버플로 0) 확인. 인위적으로 만든 최악의 조합(7자 닉네임 + 2025년
  12월 + "오늘" 버튼 노출)도 오버플로 0으로 확인(스크린샷으로 레이아웃도
  안 깨지는 것 확인 — 극단적 상황에선 닉네임이 화살표만 남을 정도로
  줄어들지만 페이지는 절대 안 넘침)
- `tsc -b`/`oxlint`/`npm run build` 통과

## 2026-07-21 (78차) — 77차(줌 비활성화) 되돌림 + 실기기 기반 진단으로 방향 전환

사용자가 77차 접근(확대/축소 비활성화)을 명확히 반려: "줌을 못쓰는건 이
프로젝트 취지랑 안맞아 이거 새로운 방식으로 접근해야됨". 추가로 중요한
단서 제공: "세로모드로 봤을때 폴드가 폭이 훨씬 좁은데 문제없이 출력이
된단 말이야 이거는 비율과 사이즈 문제야" — 갤럭시 폴드(커버 화면, 폭이
아이폰보다 훨씬 좁음)는 문제없이 나오는데 아이폰 13프로만 깨진다는 것 —
"좁을수록 더 깨진다"가 아니라 아이폰 13프로의 특정 크기/비율에서만
발생한다는 뜻이라 단순 좁은 뷰포트 오버플로 이론과는 안 맞음.

### 되돌림
- `index.html` — 77차에서 추가한 `maximum-scale=1.0, minimum-scale=1.0,
  user-scalable=no` 제거, `width=device-width, initial-scale=1.0`로 원복

### 재현 시도 — 이 환경의 한계
- Chromium(Playwright)으로 아이폰 13프로 뷰포트(390×844)에서 홈/계산기/
  정산 표 전부 확인했지만 가로 오버플로 재현 안 됨(77차에서 이미 확인)
- 실제 iOS Safari(WebKit) 엔진 재현 시도: Playwright의 webkit 브라우저는
  다운로드가 너무 오래 걸려 포기(사용자 요청으로 중단). 시스템에 이미
  설치된 WebKit2GTK(리눅스용 WebKit, 데스크탑 GNOME 브라우저용)를
  파이썬 GObject 바인딩으로 직접 구동해보려 했으나, 이 컨테이너 환경에
  Xvfb/디스플레이 서버가 없고(sudo 권한도 없어 설치 불가) GTK의
  offscreen 렌더링 백엔드도 컴파일되어 있지 않아(GDK_BACKEND=offscreen
  실패, 지원 백엔드는 wayland/x11/broadway뿐) 실패 — 이 환경에서는
  실제 WebKit 엔진으로 렌더링을 재현할 방법이 없다고 결론
- Tailwind v4가 생성하는 CSS의 `sm:`/`lg:` 브레이크포인트가
  `rem` 단위(`width>=40rem` 등)로 컴파일되는 것을 확인 — 기기의 실효
  루트 폰트 크기(Safari 텍스트 크기 접근성 설정, 사이트별 확대 설정 등)에
  따라 브레이크포인트 트리거 지점이 달라질 수 있다는 이론을 세웠으나,
  문제가 보고된 계산기 화면은애초에 `sm:`/`lg:` 반응형 클래스를 전혀
  안 써서 이 이론으로는 해당 화면의 증상을 설명할 수 없어 기각

### 다음 방향 — 코드 추측 대신 실기기에서 직접 확인
- 더 이상 이 환경에서 원인을 특정할 근거가 없어, 사용자가 실기기에서
  직접 확인 가능한 진단 절차로 전환(사용자에게 안내):
  1. 같은 iPhone에서 사파리가 아닌 다른 브라우저 앱(크롬 등, iOS는
     내부적으로 전부 WebKit을 쓰게 강제되지만 브라우저 앱마다 자체
     설정/캐시/사이트별 확대 기억은 다름)으로 같은 주소 접속 시에도
     동일하게 깨지는지 확인 — 사파리에서만 깨지면 엔진 자체 버그가
     아니라 사파리의 사이트별 설정(확대 비율 저장 등)일 가능성이 큼
  2. 사파리 주소창 "aA" 버튼 → 현재 페이지 확대 비율이 100%가
     아닌 값으로 저장돼 있는지 확인, 있다면 100%로 재설정
  3. 그래도 재현되면 화면이 깨지는 "순간"을 화면 녹화로 남겨서 공유
     요청(정적 사진보다 훨씬 진단에 유리)

## 2026-07-20 (76차) — 가로 회전 시 CSS로 세로 강제(75차 후속)

75차에서 manifest의 `orientation: portrait` 고정을 지웠더니 화면 깨짐은
해결됐지만(사용자 확인), 새로운 증상 제보: 홈 화면에서 좌우로 스와이프
(월 이동 제스처)하다가 아이폰이 실제로 가로 모드로 돌아가 버림(사진
확인 결과 스와이프에 의한 월 이동 자체는 정상 동작 — 7월→6월로 바뀜.
다만 화면이 가로로 표시됨). 한손으로 스와이프하며 손목이 살짝 기울면
실제 기기 자이로가 가로로 인식하는 것으로 추정 — 이 앱은 세로 전용
레이아웃이라 가로에서는 안 예쁘게 보임.

### 설계
- iOS Safari는 `screen.orientation.lock()` JS API를 아예 지원하지
  않고, manifest `orientation` 필드도 75차에서 확인했듯 PWA에서
  오히려 렌더링 깨짐을 유발할 수 있어 신뢰할 수 없음 — 두 네이티브
  방식 다 배제
- 대신 웹에서 널리 쓰이는 순수 CSS 트릭 사용: 뷰포트가 실제로 가로
  방향이 되면(`@media (orientation: landscape)`), `body`를
  `transform: rotate(-90deg)` + `width: 100vh; height: 100vw` +
  `position: absolute`로 강제로 다시 세로처럼 보이게 회전시킴. 기기
  자체는 가로로 인식하지만 화면에는 항상 세로 레이아웃만 보이게 됨
- 태블릿/데스크탑의 정상적인 가로 사용까지 건드리지 않도록
  `max-width: 950px`(아이폰 중 가장 큰 가로 폭인 Pro Max 932pt보다
  약간 여유)로 폰 크기에서만 적용되게 제한
- manifest.json의 `orientation` 필드는 75차에서 지운 상태 그대로
  유지(다시 넣지 않음) — iOS의 네이티브 방향 고정 시도 자체가 75차
  버그의 원인으로 추정되므로, 이번엔 순수 CSS만으로 방향을 처리해
  같은 충돌이 재발하지 않게 함

### 계획
- `src/index.css` — 가로 모드(폰 너비 한정) 시 body를 CSS로
...
보임(사진 첨부, 폰은 정상 세로 상태). 안드로이드 등 다른 폰에서는 문제
없음.

### 원인 분석
- `public/manifest.json`에 `"orientation": "portrait"`로 화면 방향을
  강제 고정해뒀음. iOS Safari/WebKit은 PWA(홈 화면 추가, standalone
  모드)의 manifest `orientation` 필드 지원이 오래전부터 버전별로
  불안정하고 버그가 있는 것으로 알려져 있음 — 강제 고정 값과 실제
  기기 상태가 어긋나면 콘텐츠 렌더링이 찌그러지는 증상으로 이어질 수
  있음. 안드로이드 크롬은 이 필드를 문제없이 처리해서 증상이 이쪽에만
  나타난 것으로 보임
- 앱은 이미 Tailwind 반응형 클래스로 모바일 레이아웃을 전부 처리하고
  있어(이번 세션에서 계속 확인된 단일 컬럼 모바일 UI) `orientation`
  강제 고정이 기능적으로 필요하지 않음 — 제거해도 CSS가 알아서
  반응형을 유지하므로 안전한 수정
- **주의**: iOS는 PWA를 홈 화면에 추가하는 시점에 manifest를 캡처해두는
  경향이 있어, 이 수정을 배포해도 기존에 이미 추가해둔 아이콘은 캐시된
  옛 manifest를 계속 쓸 수 있음 — 사용자가 기존 홈 화면 아이콘을 삭제하고
  다시 "홈 화면에 추가"해야 반영될 가능성이 높음(WORKLOG에도 명시, 사용자
  안내 필요)

### 계획
- `public/manifest.json` — `orientation` 필드 제거
- 로컬에서 육안 검증은 iOS 기기가 없어 한계가 있음 — 코드/설정 변경
